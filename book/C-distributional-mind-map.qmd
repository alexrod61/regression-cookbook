...
- **Lognormal.** A `r colourize("random variable", "blue")` $Y$ with a support of $y \in [0, \infty)$, whose transformation $\log{(Y)}$ yields a Normal distribution. It is used in waiting time modelling for some event of interest to happen (i.e., **survival times**). Its distributional `r colourize("parameters", "blue")` are the **Normal location** `r colourize("continuous parameter", "blue")` $\mu \in (-\infty, \infty)$ and **Normal scale** `r colourize("continuous parameter", "blue")` $\sigma^2 \in (0, \infty)$.
...
- **Gamma** A `r colourize("random variable", "blue")` $Y$ with a support of $y \in [0, \infty)$. It is used in waiting time modelling for some event of interest to happen (i.e., **survival times**). Its distributional `r colourize("parameters", "blue")` are the **shape** `r colourize("continuous parameter", "blue")` $\eta \in (0, \infty)$ and **scale** `r colourize("continuous parameter", "blue")` $\theta \in (0, \infty)$.
...
`r colourize("Parameter", "blue")` $\lambda \in (0, \infty)$ refers to the random process' rate. @fig-exponential-family-rate shows three members of the Exponential parametric family, i.e., three different `r colourize("PDFs", "blue")` with different rate parameters $\lambda = 0.25, 0.5, 1$. **As we increase the rate parameter**, note that smaller observed values $y$ get more probable.
