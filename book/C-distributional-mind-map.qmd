...
Suppose you observe the count of events happening in a **fixed interval of time or space**, for instance. Let $Y$ be the number of counts considered of integer type. Then, $Y$ is said to have a **classical Poisson distribution** with a `r colourize("continuous parameter", "blue")` $\lambda$:
...
where $\exp{(\cdot)}$ depicts the base $e$ (also known as **Euler's number**, $e = 2.71828...$) and $y!$ is the factorial
...
The `r colourize("continuous parameter", "blue")` $\lambda \in (0, \infty)$ represents the average rate at which these events happen (e.g., events per area unit or events per time unit). Curiously, even though the `r colourize("random variable", "blue")` $Y$ is considered `r colourize("discrete", "blue")` in this case, $\lambda$ is modelled as `r colourize("continuous", "blue")`!
...
![Portrait of mathematician Brook Taylor [@earlom-1793].](img/brook-taylor.jpg){width="300"}
...
$$
\begin{align*}
\mathbb{E} \left[ Y (Y - 1) \right] &= \sum_{y = 0}^n y (y - 1) P \left( Y = y \mid \lambda \right) \\
&= \sum_{y = 2}^n y (y - 1) P \left( Y = y \mid \lambda \right) \\
& \quad \qquad \text{for $y = \{0, 1\}$,} \\ 
& \quad \qquad \text{the addends are equal to zero} \\
&= \sum_{y = 2}^n y (y - 1) \left[ \frac{\lambda^y \exp{(-\lambda)}}{y!} \right] \\
&= \exp{(-\lambda)} \sum_{y = 2}^n \left[ \frac{y (y - 1) \lambda^y}{y!} \right] \\
& \quad \qquad \text{factoring out $\exp{(-\lambda)}$,} \\
& \quad \qquad \text{since it does not depend on $y$} \\
&= \exp{(-\lambda)} \sum_{y = 2}^n \left[ \frac{y (y - 1) \lambda^y}{y (y - 1) (y - 2)!} \right] \\
& \quad \qquad \text{in the denominator, $y! = y (y - 1) (y - 2)!$} \\
&= \exp{(-\lambda)} \sum_{y = 2}^n \frac{\lambda^y}{(y - 2)!} \\
&= \exp{(-\lambda)} \sum_{y = 2}^n \frac{\lambda^{y + 2 - 2}}{(y - 2)!} \\
& \quad \qquad \text{note $\lambda^y = \lambda^{y + 2 - 2} $} \\
&= \lambda^2 \exp{(-\lambda)} \sum_{y = 2}^n \frac{\lambda^{y - 2}}{(y - 2)!} \\
