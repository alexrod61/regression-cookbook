...
# Distributional Mind Map {#sec-distributional-mind-map}
...
Note that when $g(Y) = y$ in the discrete case, @eq-app-expected-value-discrete-function becomes

$$
\mathbb{E} \left( Y \right) = \displaystyle \sum_{y \in \mathcal{Y}} y \cdot P_Y(Y = y).
$${#eq-app-expected-value-discrete}

On the other hand, when $g(Y) = y$ in the continuous case, @eq-app-expected-value-continuous-function becomes
...
Now, for each case depicted as a cloud in @fig-app-distributions, subsequent sections in this appendix will show elaborate on why each corresponding PMF or PDF (depending on the type of random variable, $Y$) is a proper probability distribution (i.e., all the standalone probabilities over the support of $Y$ add up to one) along with the respective proofs of their corresponding means and variances.

# Discrete Random Variables

Let us recall what a discrete random variable is. This type of variable is defined to take on a set of countable possible values. In other words, these values belong to a finite set. @fig-app-distributions delves into the following specific probability distributions:
...
@tbl-distributions-discrete outlines the parameter(s), support, mean, and variance for each discrete probability distribution utilized to model the target $Y$ in a specific regression tool explained in this book.
...
Note that the support of $Y$ in @eq-app-bernoulli-support makes it binary with these outcomes: $1$ for *success* and $0$ for *failure*.  Then, $Y$ is said to have a **Bernoulli distribution** with parameter $\pi$:
...
Parameter $\pi \in [0, 1]$ refers to the probability of success. We can verify @eq-app-bernoulli-pmf is a proper probability distribution (i.e., all the standalone probabilities over the support of $Y$ add up to one) given that:
...
Via @eq-app-expected-value-discrete, the expected value or mean of a Bernoulli-distributed random variable $Y$ can be found as follows:
...
Via @eq-app-variance and the @eq-app-expected-value-discrete of a discrete expected value, the variance of a Bernoulli-distributed random variable $Y$ can be found as follows:
...
> **How can we verify that @eq-app-binomial-pmf is a proper PMF (i.e., all the standalone probabilities over the support of $Y$ add up to one)?**
...
After the above fruitful digression on the binomial theorem, let us use it to show that our Binomial PMF in @eq-app-binomial-pmf actually adds up to one all over the support of the random variable:
...
Via @eq-app-expected-value-discrete, the expected value or mean of a Binomial-distributed random variable $Y$ can be found as follows:
...
Note that, in the summation of @eq-proof-binomial-mean-2, we encounter the PMF of a random variable $Z$ as follows:

$$
Z \sim \text{Bin}(m, \pi).
$$

Since the summation, where this Binomial PMF of $Z$ is depicted, goes from $z = 0$ to $m$, we can apply our result from @eq-proof-binomial-PMF-adds-to-1:
...
Via @eq-app-variance and the @eq-app-expected-value-discrete of a discrete expected value, the variance of a Binomial-distributed random variable $Y$ can be found as follows:
...
Note that, in the summation of @eq-proof-binomial-variance-4, we encounter the PMF of a random variable $Z$ as follows:
...
::::
::::{.Tip-container}
There is an alternative parametrization to define a Negative Binomial distribution in which we have a random variable $Z$ defined as the **total number of Bernoulli trials** (i.e., $k$ successes plus the $Y$ failures depicted in @eq-app-neg-binomial-pmf):
...
Nevertheless, we will not dig into this version of the Negative Binomial distribution since @sec-negative-binomial delves into a modelling estimation via a joint PMF of the training set involving @eq-app-neg-binomial-pmf.
::::
:::

> **How can we verify that @eq-app-neg-binomial-pmf is a proper PMF (i.e., all the standalone probabilities over the support of $Y$ add up to one)?**
...
Now, let us begin with the summation involving the Negative Binomial PMF depicted in  @eq-app-neg-binomial-pmf from $0$ to $\infty$:
...
Via @eq-app-expected-value-discrete, the expected value or mean of a Negative Binomial-distributed random variable $Y$ can be found as follows:
...
Note that, in the summation of @eq-proof-neg-binomial-mean-2, we encounter the PMF of a random variable $Z$ as follows:

$$
Z \sim \text{NegBin}(m, \pi).
$$

Since the summation, where this Negative Binomial PMF of $Z$ is depicted, goes from $z = 0$ to $\infty$, we can apply our result from @eq-proof-neg-binomial-PMF-adds-to-1-c:
...
Via @eq-app-variance and the @eq-app-expected-value-discrete of a discrete expected value, the variance of a Negative Binomial-distributed random variable $Y$ can be found as follows:
...
Note that, in the summation of @eq-proof-neg-binomial-variance-4, we encounter the PMF of a random variable $Z$ as follows:

$$
Z \sim \text{NegBin}(m, \pi).
$$

Since the summation, where this Binomial PMF of $Z$ is depicted, goes from $z = 0$ to $\infty$, we can apply our result from @eq-proof-neg-binomial-PMF-adds-to-1-c:
...
> **How can we verify that @eq-app-classical-poisson-pmf is a proper PMF (i.e., all the standalone probabilities over the support of $Y$ add up to one)?**
...
The above Mclaurin series in @eq-maclaurin-series will help us to show that our Poisson PMF in @eq-app-classical-poisson-pmf actually adds up to one all over the support of the random variable:
...
Via @eq-app-expected-value-discrete, the expected value or mean of a Poisson-distributed random variable $Y$ can be found as follows:
...
Via @eq-app-variance and the @eq-app-expected-value-discrete of a discrete expected value, the variance of a Poisson-distributed random variable $Y$ can be found as follows:
...
1. When $0 < \theta < 1$, the GP $Y$ shows overdispersion which implies that $$\text{Var}(Y) > \mathbb{E}(Y).$$

2. When $-1 < \theta < 0$, the GP $Y$ shows underdispersion which implies that 
$$\text{Var}(Y) < \mathbb{E}(Y).$$

3. When $\theta = 0$, the PMF of the GP $Y$ in @eq-app-generalized-poisson-pmf becomes the classical Poisson PMF from @eq-app-classical-poisson-pmf:
$$
\begin{align*}
...
> **How can we verify that @eq-app-generalized-poisson-pmf is a proper PMF (i.e., all the standalone probabilities over the support of $Y$ add up to one)?**
...
Let us recall what a continuous random variable is. This type of variable is defined to take on a set of uncountable possible values. In other words, these values belong to an infinite set. @fig-app-distributions delves into the following specific probability distributions:
...
| **Pareto** as in  | $$y \in [0, \infty)$$ | |  |
| **Lognormal** as in <br> $Y \sim \text{Lognormal}(\mu, \sigma^2)$ with Normal location <br> $\mu \in (-\infty, \infty)$ <br> and Normal scale <br> $\sigma^2 \in (0, \infty)$ | $$y \in [0, \infty)$$  | $$\exp{\left[ \mu + \left( \frac{\sigma^2}{2} \right) \right]}$$ | $$\exp{\left[ 2 \left( \mu + \sigma^2 \right) \right]} - $$ $$ \quad \exp{\left( 2 \mu + \sigma^2 \right)}$$ |
...
| **Gamma** as in <br> $Y \sim \text{Gamma}(\eta, \theta)$ with shape <br> $\eta \in (0, \infty)$ <br> and scale <br> $\theta \in (0, \infty)$ | $$y \in [0, \infty)$$ | $$\eta \theta$$ | $$\eta \theta^2$$ |
...
#| fig-cap: "Some members of the Weibull parametric family."
#| warning: false
#| echo: false
#| fig.height: 13
#| fig.width: 14
...
As a side note, the Weibull and Exponential PDFs are mathematically related. When $\gamma = 1$ in @eq-app-weibull-pdf, the Weibull PDF is equal to the Exponential PDF under the **scale parametrization** as in @eq-app-exponential-pdf-scale:
...
> **How can we verify that @eq-app-weibull-pdf is a proper PDF (i.e., @eq-app-weibull-pdf integrates to one over the support of $Y$)?**
...
The above rearrangement yields the following in @eq-proof-weibull-PMF-adds-to-1-1:
...
Via @eq-app-expected-value-continuous, the expected value or mean of a Weibull-distributed random variable $Y$ can be found as follows:
...
The above rearrangement yields the following in @eq-proof-weibull-mean-1:
...
Via @eq-app-variance and the @eq-app-expected-value-continuous of a continuous expected value, the variance of a Weibull-distributed random variable $Y$ can be found as follows:
...
The above rearrangement yields the following in @eq-proof-weibull-variance-2:
...
## Pareto {#sec-pareto-distribution}
...
Parameter $\lambda \in (0, \infty)$ refers to the random process' rate. @fig-exponential-family-rate shows three members of the Exponential parametric family, i.e., three different PDFs with different rate parameters $\lambda = 0.25, 0.5, 1$. **As we increase the rate parameter**, note that smaller observed values $y$ get more probable.

```{r}
#| label: fig-exponential-family-rate
#| fig-cap: "Some members of the Exponential parametric family with rate parametrization."
#| warning: false
#| echo: false
#| fig.height: 6.5
#| fig.width: 14
...
> **How can we verify that @eq-app-exponential-pdf-rate is a proper PDF (i.e., @eq-app-exponential-pdf-rate integrates to one over the support of $Y$)?**
...
Parameter $\beta \in (0, \infty)$ refers to the random process' scale. @fig-exponential-family-scale shows three members of the Exponential parametric family, i.e., three different PDFs with different scale parameters $\beta = 0.25, 0.5, 1$. **As we increase the scale parameter**, note that larger observed values $y$ get more probable.

```{r}
#| label: fig-exponential-family-scale
#| fig-cap: "Some members of the Exponential parametric family with scale parametrization."
#| warning: false
#| echo: false
#| fig.height: 6.5
#| fig.width: 14
...
> **How can we verify that @eq-app-exponential-pdf-scale is a proper PDF (i.e., @eq-app-exponential-pdf-scale integrates to one over the support of $Y$)?**
...
Via @eq-app-expected-value-continuous, the expected value or mean of an Exponential-distributed random variable $Y$ can be found as follows:
...
Via @eq-app-variance and the @eq-app-expected-value-continuous of a continuous expected value, the variance of an Exponential-distributed random variable $Y$ can be found as follows:
...
::::
::::{.Heads-up-container}
The term $\pi$ in the Normal PDF depicted in @eq-app-normal-pdf corresponds to the mathematical constant $3.141592...$ Hence, this term **does not** correspond to another population parameter in this probability distribution.
::::
:::

Parameters $\mu \in (-\infty, \infty)$ and $\sigma \in (0, \infty)$ refer to the random process' location and scale, respectively. @fig-normal-family shows nine members of the Normal parametric family, i.e., nine different PDFs with all possible pairwise combinations for three different scale parameters $\mu = -3, 0, 3$ and shape parameters $\sigma^2 = 0.25, 1, 4$. We can highlight the following:
...
#| fig-cap: "Some members of the Normal or Gaussian parametric family."
#| warning: false
#| echo: false
#| fig.height: 13
#| fig.width: 14
...
> **How can we verify that @eq-app-normal-pdf is a proper PDF (i.e., @eq-app-normal-pdf integrates to one over the support of $Y$)?**
