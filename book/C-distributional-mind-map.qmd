...
- **Exponential.** A `r colourize("random variable", "blue")` $Y$ with a support of $y \in [0, \infty)$, which is often used to model waiting times for some event of interest to happen (i.e., **survival times**). Its single distributional `r colourize("parameter", "blue")` can come in either one of the following forms:
...
\begin{align*}
\int_{y = 0}^{y = \infty} f_Y \left(y \mid \beta, \gamma \right) \mathrm{d}y &= \int_{y = 0}^{y = \infty} \frac{\gamma}{\beta} \left( \frac{y}{\beta} \right)^{\gamma - 1} \exp{\left[ \left( -\frac{y}{\beta} \right)^{\gamma} \right]} \mathrm{d}y \\
&= \int_{u = 0}^{u = \infty} \frac{\gamma}{\beta} \left( \frac{\beta u^{\frac{1}{\gamma}}}{\beta} \right)^{\gamma - 1} \exp{\left( -u \right)} \frac{\beta}{\gamma} u^{\frac{1}{\gamma} - 1} \mathrm{d}u \\
...
$$
\begin{align*}
\mathbb{E}(Y) &= \frac{\gamma}{\beta^{\gamma}} \int_{y = 0}^{y = \infty} y^{\gamma} \exp{\left[ -\left( \frac{y}{\beta} \right)^{\gamma} \right]} \mathrm{d}y \\
&= \frac{\gamma}{\beta^{\gamma}} \int_{u = 0}^{u = \infty} \left( \beta u^{\frac{1}{\gamma}} \right)^{\gamma} \exp{\left( -u \right)} \frac{\beta}{\gamma} u^{\frac{1}{\gamma} - 1} \mathrm{d}u \\
...
\begin{align*}
\text{Var} (Y) &= \mathbb{E}(Y^2) - \left[ \mathbb{E}(Y)\right]^2 \\
&= \mathbb{E}(Y^2) - \beta^2 \Gamma^2 \left( \frac{1}{\gamma} + 1 \right) \\
& \quad \qquad \text{since $\mathbb{E}(Y) = \beta \Gamma \left( \frac{1}{\gamma} + 1 \right)$}.
\end{align*}
$${#eq-proof-weibull-variance-1}

Now, we need to find $\mathbb{E}(Y^2)$ from @eq-proof-weibull-variance-1. Thus, we make the following derivation via the LOTUS from @eq-app-expected-value-continuous-function when $g(Y) = y^2$:
...
$$
\begin{align*}
\mathbb{E} \left( Y^2 \right) &= \frac{\gamma}{\beta^{\gamma}} \int_{y = 0}^{y = \infty} y^{\gamma + 1} \exp{\left[ -\left( \frac{y}{\beta} \right)^{\gamma} \right]} \mathrm{d}y \\
&= \frac{\gamma}{\beta^{\gamma}} \int_{u = 0}^{u = \infty} \left( \beta u^{\frac{1}{\gamma}} \right)^{\gamma + 1} \exp{\left( -u \right)} \frac{\beta}{\gamma} u^{\frac{1}{\gamma} - 1} \mathrm{d}u \\
...
\begin{align*}
\text{Var} (Y) &= \mathbb{E}(Y^2) - \left[ \mathbb{E}(Y)\right]^2 \\
&= \mathbb{E}(Y^2) - \frac{1}{\lambda^2} \qquad \text{since $\mathbb{E}(Y) = \frac{1}{\lambda}$}.
\end{align*}
$${#eq-proof-exponential-rate-variance-1}

Now, we need to find $\mathbb{E}(Y^2)$ from @eq-proof-exponential-rate-variance-1. Hence, we make the following derivation via the LOTUS from @eq-app-expected-value-continuous-function when $g(Y) = y^2$:

$$
\begin{align*}
\mathbb{E}(Y^2) &= \int_{y = 0}^{y = \infty} y^2 f_Y \left(y \mid \lambda \right) \mathrm{d}y \\
...
$$
\begin{align*} 
\mathbb{E}(Y^2) &= \lambda \left[ u v \Bigg|_{y = 0}^{y = \infty} - \int_{y = 0}^{y = \infty} v \mathrm{d}u \right] \\
...
$$
\begin{align*} 
\mathbb{E}(Y^2) &= 2 \int_{y = 0}^{y = \infty} y \exp{\left( -\lambda y \right)} \mathrm{d}y \\
&= 2 \left[ u v \Bigg|_{y = 0}^{y = \infty} - \int_{y = 0}^{y = \infty} v \mathrm{d}u \right] \\
...
$$
\begin{align*}
\text{Var} (Y) &= \mathbb{E}(Y^2) - \frac{1}{\lambda^2} \\
...
\begin{align*}
\text{Var} (Y) &= \mathbb{E}(Y^2) - \left[ \mathbb{E}(Y)\right]^2 \\
&= \mathbb{E}(Y^2) - \beta^2 \qquad \text{since $\mathbb{E}(Y) = \beta$}.
\end{align*}
$${#eq-proof-exponential-scale-variance-1}

Now, we need to find $\mathbb{E}(Y^2)$ from @eq-proof-exponential-scale-variance-1. Hence, we make the following derivation via the LOTUS from @eq-app-expected-value-continuous-function when $g(Y) = y^2$:

$$
\begin{align*}
\mathbb{E}(Y^2) &= \int_{y = 0}^{y = \infty} y^2 f_Y \left(y \mid \beta \right) \mathrm{d}y \\
...
$$
\begin{align*} 
\mathbb{E}(Y^2) &= \frac{1}{\beta} \left[ u v \Bigg|_{y = 0}^{y = \infty} - \int_{y = 0}^{y = \infty} v \mathrm{d}u \right] \\
...
$$
\begin{align*} 
\mathbb{E}(Y^2) &= 2 \int_{y = 0}^{y = \infty} y \exp \left( -\frac{y}{\beta} \right) \mathrm{d}y \\
&= 2 \left[ u v \Bigg|_{y = 0}^{y = \infty} - \int_{y = 0}^{y = \infty} v \mathrm{d}u \right] \\
