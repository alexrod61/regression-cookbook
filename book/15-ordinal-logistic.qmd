...
We first encode each individual’s observed response category using indicator variables. Let $y_{i,j} = 1$ if the $i$-th individual belongs to category $j$, and let $y_{i,j} = 0$ otherwise. Since each individual belongs to exactly one of the (J) possible categories, the vector $(y_{i,1}, y_{i,2}, \ldots, y_{i,J})$ contains exactly one entry equal to (1), with all remaining entries equal to (0). This representation is often called a one-hot encoding of the response category. The likelihood function for the ordinal logistic regression model is the following:
