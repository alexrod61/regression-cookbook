...
- **Inference**: Estimate the association between explanatory variables—specifically continuous GPA and binary parental education status—and the cumulative odds of a student belonging to a higher "likelihood" category.
- **Prediction**: Develop a classification model that classifies individuals into their most likely ordinal category for the decision to apply to graduate school: `unlikely`, `somewhat likely`, or `very likely`.
...
In this example, variable `decision` will be our response of interest. Let's set it up as an **ordered factor** via the `as.ordered()` function.
...
Box and violin plots, along with stacked bar charts, illustrate how academic performance and parental education background relate to a student's intent to apply to graduate school.
...
- The mean GPA, represented by the red diamonds in the violin plots, shows a consistent upward trend as the level of intent increases, suggesting a positive association between academic achievement and graduate school aspirations. 
...
- Among students whose parents do not have a graduate degree, a clear majority (59%) report being `unlikely` to apply. In contrast, for students with at least one parent holding a graduate degree, the proportion of those `unlikely` to apply drops significantly to 32%. Notably, the proportion of students who are `very likely` to apply nearly triples—from 8% to 21%—when at least one parent has a graduate degree, indicating that parental academic background may influence a student's own educational goals.
