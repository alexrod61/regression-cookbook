...
|---|---|---|---|---|
| `Low\|Medium` ($j=1$) | 0.066 | 1.068 | (1.017, 1.123) | 1.391 |
| `Medium\|High` ($j=2$) |0.174 | 1.190 | (1.126, 1.257) | 2.387 |
| `High\|Very High` ($j=3$) | 0.275 | 1.317 | (1.217, 1.425) | 3.955 |

The increasing pattern across cutpoints is the core finding. At the `Low|Medium` split, a one-point GRE increase is associated with only 1.068 times higher odds of being above `Low`, a modest effect. But at the `High|Very High` split, the same one-point increase is associated with 1.317 times higher odds of being in the `Very High` category. Expressed as 5-point effects, the contrast is even starker: a 5-point GRE advantage multiplies the odds by only 1.391 at the lowest split, but by nearly 4.0 at the highest. This confirms that GRE score is primarily a discriminator for students at the top of the ordinal scale rather than a uniform regressor across the full range.

**Field of Study (`field_of_study`), shared slopes.** None of the field-of-study levels reaches statistical significance at the 5% level: Business ($\hat{\beta}_{\texttt{Business}} = 0.06$, $z = 0.12$, $p = 0.907$, OR = 1.06), Humanities ($\hat{\beta}_{\texttt{Humanities}} = -0.28$, $z = -0.49$, $p = 0.626$, OR = 0.76), STEM ($\hat{\beta}_{\texttt{STEM}} = 0.13$, $z = 0.26$, $p = 0.794$, OR = 1.14), and Social Sciences ($\hat{\beta}_{\texttt{Social Sciences}} = -0.23$, $z = -0.43$, $p = 0.666$, OR = 0.80). After adjusting for GPA and GRE score, a student's field of study shows no statistically detectable association with their inclination to apply to graduate school relative to the `Arts` baseline.
...
|---|---|---|---|---|---|---|
| $\hat{\alpha}_1$ (`Low\|Medium`) | 21.10 | — | — | — | — | — |
| $\hat{\alpha}_2$ (`Medium\|High`) | 38.78 | — | — | — | — | — |
| $\hat{\alpha}_3$ (`High\|Very High`) | 55.46 | — | — | — | — | — |
| $\hat{\beta}_{\texttt{GPA}}$ (shared) | 4.20 | 0.545 | 7.71 | $< 0.001$ | 66.69 | (22.95, 194.22) |
| $\hat{\beta}_{\texttt{GRE},1}$ (`Low\|Medium`) | 0.066 | 0.025 | 2.62 | 0.009 | 1.068 | (1.017, 1.123) |
| $\hat{\beta}_{\texttt{GRE},2}$ (`Medium\|High`) | 0.174 | 0.028 | 6.22 | $< 0.001$ | 1.190 | (1.126, 1.257) |
| $\hat{\beta}_{\texttt{GRE},3}$ (`High\|Very High`) | 0.275 | 0.040 | 6.82 | $< 0.001$ | 1.317 | (1.217, 1.425) |
| $\hat{\beta}_{\texttt{Business}}$ (shared) | 0.06 | 0.508 | 0.12 | 0.907 | 1.06 | (0.39, 2.87) |
| $\hat{\beta}_{\texttt{Humanities}}$ (shared) | -0.28 | 0.566 | -0.49 | 0.626 | 0.76 | (0.25, 2.30) |
| $\hat{\beta}_{\texttt{STEM}}$ (shared) | 0.13 | 0.509 | 0.26 | 0.794 | 1.14 | (0.42, 3.10) |
| $\hat{\beta}_{\texttt{Social Sciences}}$ (shared) | -0.23 | 0.521 | -0.43 | 0.666 | 0.80 | (0.29, 2.22) |

After the Brant–Wald test revealed that GRE score's slope varies significantly across cutpoints, the partial proportional odds model (fitted via `vglm()` in `R`) produced cutpoint-specific GRE slopes of $\hat{\beta}_{\texttt{GRE},1} = 0.066$, $\hat{\beta}_{\texttt{GRE},2} = 0.174$, and $\hat{\beta}_{\texttt{GRE},3} = 0.275$ at the three cumulative splits respectively, with all other slope coefficients remaining shared. The interpretation of each significant coefficient is:

- **GPA** ($\hat{\beta} = 4.20$, OR = 66.69, 95% CI: 22.95–194.22): Holding GRE score and field of study constant, each additional GPA point is associated with 66.69 times higher cumulative odds of being in a higher likelihood-to-apply category, at every cumulative threshold simultaneously. This is by far the strongest regressor in the model.
- **GRE Score** (cutpoint-specific): The effect of GRE score on the cumulative log-odds grows substantially across cutpoints. A one-point increase in GRE score is associated with a 6.8% increase in the odds of being above `Low` (OR = 1.068 at $j=1$), rising to a 19.0% increase in the odds of being above `Medium` (OR = 1.190 at $j=2$), and a 31.7% increase in the odds of being in the `Very High` category (OR = 1.317 at $j=3$). This confirms that GRE score primarily distinguishes students at the top of the ordinal scale.

None of the field-of-study levels is statistically significant after adjusting for GPA and GRE score (all $p > 0.6$), so field of study does not contribute a detectable shift in the cumulative odds relative to the `Arts` baseline.
