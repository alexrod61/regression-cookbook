...
When the proportional odds assumption is violated for one or more regressors, we allow each cumulative logit to have its own set of regression coefficients. We can formally check the proportional odds assumption using Brant-Wald test, which will be introduced in the Inference section (@brant-wald). Recall the non-proportional odds model introduced in the Data Modeling section (@eq-npo), where every slope $\beta_{k,j}$ now depends on the cumulative split $j$. In the case study model with regressors $x_{i,\texttt{GPA}}$, $x_{i,\texttt{GRE}}$, and the `field_of_study` dummy variables $\mathbf{Z}_{i,\texttt{field}}$, the fitted non-proportional odds model can be written as:
...
### Brant–Wald Test for Proportional Odds {#brant-wald}
