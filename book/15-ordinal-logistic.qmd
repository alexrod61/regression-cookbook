...
- The outcome variable is continuous or a count. For continuous and unbounded outcomes, refer to ordinary least-squares regression (@sec-ols); for count outcomes, refer to the count regression models, such as Poisson regression (see @sec-generalized-poisson).
...
- There should be one or more independent variables that are continuous, ordinal, or nominal.
...
- If the proportional odds assumption is violated, a **non-proportional odds** model should be used because it allows for some regressors to vary across thresholds.
...
|---|---|---|---|---|---|
| Inclination to Apply to Graduate School | How likely the student will apply to graduate school, with four ordered categories | Discrete | Ordinal (`Low` < `Medium` < `High` < `Very High`) | Response | `inclination_to_apply` |
...
In this example, the variable `inclination_to_apply` will be our response of interest. Before modelling, we explicitly tell `R` and `Python` that this variable is ordinal. This matters for two reasons. First, ordinal logistic regression is built on the *cumulative* probability of being at or below each category, so the software needs to know the correct ordering (`Low` < `Medium` < `High` < `Very High`) in order to form the correct cumulative splits. Second, if the categories were left as unordered nominal factors, the model would ignore their natural ranking and discard exactly the structure that we would like to model.
...
For example, let's consider $J = 3$ ordered categories, such that $j$ runs over the values $1, 2, \ldots, J - 1 = 2$, and therefore there are two cumulative probabilities to consider:
...
The residual deviance (2049.12) is smaller than the null deviance (2201.55), a reduction of 152.43 units. This reduction is captured by the likelihood-ratio statistic $G^2 = 152.43$ on 6 degrees of freedom ($p < 0.001$), providing very strong evidence that at least one regressor improves on the null model. McFadden's $R^2 = 0.069$ is modest — below the 0.2–0.4 range commonly associated with a good fit for ordinal models — indicating that GPA, GRE score, and field of study together explain only a limited share of the variation in `inclination_to_apply` on the log-likelihood scale, even though their joint contribution is strongly significant.
...
**GPA (`GPA`), shared slope.** GPA retains a single slope across all three cumulative splits ($\hat{\beta}_{\texttt{GPA}} = 2.40$, $z = 5.11$, $p < 0.001$, OR = 11.01, 95% CI: 4.38, 27.63). It has the following interpretation.
...
The increasing pattern across cutpoints is the core finding. At the `Low|Medium` split, a one-point GRE increase is associated with only 1.088 times higher odds of being above `Low`, a modest effect. At the `High|Very High` split, the same one-point increase is associated with 1.145 times higher odds of being in the `Very High` category. Expressed as 5-point effects, the contrast is clearer: a 5-point GRE advantage multiplies the odds by about 1.52 at the lowest split, but by about 1.96 at the highest. This confirms that GRE score is primarily a discriminator for students at the top of the ordinal scale rather than a uniform regressor across the full range.

**Field of Study (`field_of_study`), shared slopes.** On the held-out test set, all four field-of-study levels reach significance at the 5% level relative to the `Arts` baseline: Business ($\hat{\beta}_{\texttt{Business}} = 1.11$, $z = 2.07$, $p = 0.039$, OR = 3.03), Humanities ($\hat{\beta}_{\texttt{Humanities}} = 1.08$, $z = 1.99$, $p = 0.046$, OR = 2.95), STEM ($\hat{\beta}_{\texttt{STEM}} = 1.31$, $z = 2.62$, $p = 0.009$, OR = 3.72), and Social Sciences ($\hat{\beta}_{\texttt{Social Sciences}} = 1.03$, $z = 1.97$, $p = 0.049$, OR = 2.81). These point in the same direction \u2014 students in each field show higher cumulative odds of a stronger inclination to apply than students in `Arts` \u2014 but they should be read with caution: this model is fitted on the 199-row test set, so the field coefficients are imprecise (wide confidence intervals) and noticeably larger than the corresponding training-set effects in the proportional odds model, where only `Business` was significant.
...
- **GPA** ($\hat{\beta} = 2.40$, OR = 11.01, 95% CI: 4.38–27.63): Holding GRE score and field of study constant, each additional GPA point is associated with 11.01 times higher cumulative odds of being in a higher likelihood-to-apply category, at every cumulative threshold simultaneously. This is by far the strongest regressor in the model.
- **GRE Score** (cutpoint-specific): The effect of GRE score on the cumulative log-odds grows substantially across cutpoints. A one-point increase in GRE score is associated with an 8.8% increase in the odds of being above `Low` (OR = 1.088 at $j=1$), rising to a 13.4% increase in the odds of being above `Medium` (OR = 1.134 at $j=2$), and a 14.5% increase in the odds of being in the `Very High` category (OR = 1.145 at $j=3$). This confirms that GRE score primarily distinguishes students at the top of the ordinal scale.

On the test set, all four field-of-study levels are statistically significant after adjusting for GPA and GRE score (all $p < 0.05$), with odds ratios between 2.8 and 3.7 relative to the `Arts` baseline. Because this model is fitted on the small (199-row) test set, these field effects are estimated imprecisely and run larger than the corresponding effects in the training-set proportional odds model, where only `Business` is significant; they should be interpreted with that caveat in mind.
