...
To visualize the relationship between `decision` and `parent_ed`, we will use stacked bar charts. Each bar will show the percentages (depicted on the $y$-axis) of college students falling on each ordered category of `decision` with the categories found in `parent_ed` on the $x$-axis. This class of bars will allow us to compare both levels of `parent_ed` in terms of the categories of `decision`. 
...
prop_summary = (
    college_data.groupby(['parent_ed', 'decision'])
    .size()
    .groupby(level=0)
...
pivot = prop_summary.pivot(index='parent_ed', columns='decision', values='prop').fillna(0)
