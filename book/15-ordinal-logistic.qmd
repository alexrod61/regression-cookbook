...
In `R`, we fit the fully non-proportional model using `vglm()` from the `VGAM` package. Setting `parallel = FALSE` removes the proportional odds constraint entirely. Setting `parallel = TRUE` would recover the proportional odds model, and passing a named logical vector (e.g. `parallel = c(GPA = TRUE, GRE_score = FALSE)`) fits the partial proportional odds model. For more details on the `parallel` argument, refer to the documentation on [CommonVGAMffArguments](https://www.rdocumentation.org/packages/VGAM/versions/0.9-1/topics/CommonVGAMffArguments)
...
for j, label in enumerate(cutpoints):
    ...
    # Coefficient table: estimate, standard error, z-value, and p-value per regressor
...
Both outputs now follow the same scheme: a coefficient table for each of the $J-1$ cumulative logits, reporting the estimate, standard error, z-value, and p-value for every regressor. In `R`, `vglm()` fits a single non-proportional model and prints these as stacked rows; in `Python`, where no built-in non-proportional ordinal model exists, we obtain the equivalent information by fitting one binary logistic regression per cutpoint with `sm.Logit` and printing a matching table for each. If the proportional odds assumption held perfectly, the slope estimates would be identical across cutpoints. Marked differences between them, especially relative to their standard errors, are a preliminary signal that the assumption may be violated. A formal test of this assumption is introduced in the Inference section.
...
The residual deviance (1875.57) is substantially smaller than the null deviance (2211.35), and there is a reduction of 335.78 units. This reduction is captured by the likelihood-ratio statistic $G^2 = 335.79$ on 6 degrees of freedom ($p < 0.001$), providing very strong evidence that at least one regressor improves on the null model. McFadden's $R^2 = 0.152$ falls within the 0.2 range commonly associated with acceptable fit for ordinal models, indicating that GPA, GRE score, and field of study together explain a meaningful share of the variation in `inclination_to_apply` on the log-likelihood scale.
...
On the training set, the model achieves an overall accuracy of **42.8%**, a macro recall of **40.1%**, and a macro F1 of **38.2%**. As with any multi-class ordinal problem, these figures should be interpreted relative to the random-chance baseline of 25% for a four-category outcome, so the model meaningfully outperforms chance, but leaves substantial room for improvement.
...
The `Very High` category is the best-predicted class (recall = 0.75, F1 = 0.64). The `High` category is the most difficult to classify correctly (recall = 0.11, F1 = 0.15), as it sits between two better-separated groups and receives predictions pulled in both directions. Macro recall (0.401) is noticeably lower than accuracy (0.428), indicating mild imbalance in how well the model performs across the four categories, a pattern that persists on the held-out test set and is discussed further in the Results section.
...
**GPA (`GPA`), shared slope.** GPA retains a single slope across all three cumulative splits ($\hat{\beta}_{\texttt{GPA}} = 4.20$, $z = 7.71$, $p < 0.001$, OR = 66.69, 95% CI: 22.95, 194.22). It has the following interpretation.

> *Holding GRE score and field of study constant, a one-unit increase in GPA is associated with 66.69 times higher cumulative odds of falling in a higher likelihood-to-apply category, at every cumulative threshold simultaneously.*
...
|---|---|---|---|---|
| `Low\|Medium` ($j=1$) | 0.066 | 1.068 | (1.017, 1.123) | 1.389 |
| `Medium\|High` ($j=2$) |0.174 | 1.190 | (1.126, 1.257) | 2.387 |
| `High\|Very High` ($j=3$) | 0.275 | 1.317 | (1.217, 1.425) | 3.962 |

The increasing pattern across cutpoints is the core finding. At the `Low|Medium` split, a one-point GRE increase is associated with only 1.068 times higher odds of being above `Low`, a modest effect. But at the `High|Very High` split, the same one-point increase is associated with 1.317 times higher odds of being in the `Very High` category. Expressed as 5-point effects, the contrast is even starker: a 5-point GRE advantage multiplies the odds by only 1.389 at the lowest split, but by nearly 4.0 at the highest. This confirms that GRE score is primarily a discriminator for students at the top of the ordinal scale rather than a uniform regressor across the full range.
...
for $j = 1, 2, 3$ (i.e., the three cumulative splits `Low|Medium`, `Medium|High`, and `High|Very High`). The estimated thresholds and slopes of the selected partial proportional odds model are summarised below. GPA and the field-of-study levels carry a single shared slope, while GRE score has a separate slope at each cumulative split:
...
| $\hat{\beta}_{\texttt{GPA}}$ (shared) | 3.87 | 0.24 | 16.23 | $< 0.001$ | 47.80 | (29.97, 76.25) |
| $\hat{\beta}_{\texttt{GRE},1}$ (`Low\|Medium`) | 0.044 | 0.013 | 3.40 | $< 0.001$ | 1.045 | (1.019, 1.072) |
| $\hat{\beta}_{\texttt{GRE},2}$ (`Medium\|High`) | 0.155 | 0.013 | 11.83 | $< 0.001$ | 1.167 | (1.138, 1.198) |
| $\hat{\beta}_{\texttt{GRE},3}$ (`High\|Very High`) | 0.277 | 0.017 | 16.09 | $< 0.001$ | 1.319 | (1.275, 1.364) |
| $\hat{\beta}_{\texttt{Business}}$ (shared) | 0.45 | 0.22 | 2.04 | 0.041 | 1.57 | (1.02, 2.43) |
| $\hat{\beta}_{\texttt{Humanities}}$ (shared) | 0.23 | 0.23 | 0.99 | 0.321 | 1.26 | (0.80, 1.98) |
| $\hat{\beta}_{\texttt{STEM}}$ (shared) | 0.34 | 0.22 | 1.51 | 0.130 | 1.40 | (0.90, 2.18) |
| $\hat{\beta}_{\texttt{Social Sciences}}$ (shared) | 0.25 | 0.26 | 0.97 | 0.333 | 1.28 | (0.77, 2.13) |
...
- **GPA** ($\hat{\beta} = 3.87$, OR = 47.80, 95% CI: 29.97–76.25): Holding GRE score and field of study constant, each additional GPA point is associated with 47.80 times higher cumulative odds of being in a higher likelihood-to-apply category, at every cumulative threshold simultaneously. This is by far the strongest regressor in the model.
...
- **Business field of study** ($\hat{\beta} = 0.45$, OR = 1.57, 95% CI: 1.02–2.43, $p = 0.041$): Holding academic performance constant, Business students have 1.57 times higher cumulative odds of being in a higher likelihood-to-apply category compared to Arts students. No other field of study shows a statistically significant difference from Arts.
