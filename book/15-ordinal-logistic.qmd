...
```{.python}
# Factorize the response variable and print its levels
ordered_categories = ["unlikely", "somewhat likely", "very likely"]
college_data['decision'] = pd.Categorical(
    college_data['decision'],
    categories=ordered_categories,
    ordered=True
)
print(college_data['decision'].cat.categories.tolist())
```
...
To visualize the relationship between `decision` and `GPA`, we will use the following side-by-side plots:

- **Boxplots.**
- **Violin plots.**

To visualize the relationship between `decision` and `field_of_study`, we will use stacked bar charts. Each bar will show the percentages (depicted on the $y$-axis) of college students falling on each ordered category of `decision` with the categories found in `field_of_study` on the $x$-axis. This class of bars will allow us to compare both levels of `field_of_study` in terms of the categories of `decision`. 
