...
To visualize the relationship between `likelihood_to_apply` and `field_of_study`, we will use stacked bar charts. Each bar will show the percentages (depicted on the $y$-axis) of college students falling on each ordered category of `likelihood_to_apply` with the categories found in `field_of_study` on the $x$-axis. This class of bars will allow us to compare both levels of `field_of_study` in terms of the categories of `decision`. 
