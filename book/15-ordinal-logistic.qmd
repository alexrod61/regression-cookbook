...
- How are a student's GPA and parental education status **associated** with their likelihood of applying to graduate school?
- Can we accurately **predict** a student's likelihood of applying to graduate school based on their GPA and parental education status?

These questions correspond to two distinct **statistical objectives**:
- **Inference**: Estimate the association between explanatory variables (GPA and parental education status) and the decision to apply to graduate school.
- **Prediction**: Develop a classification model that classifies individuals into their most probable ordinal category for the decision to apply to graduate school: `unlikely`, `somewhat likely`, or `very likely`.

## Data Collection and Wrangling

The dataset contains the following variables:

- `likelihood_to_apply`: how likely the student will apply to graduate school, **a discrete and ordinal response with three increasing categories** (`Low`, `Medium`, `High` and `Very High`).
...
In this example, the variable `decision` will be our response of interest. Let's set it up as an **ordered factor** via the `as.ordered()` function in R and `pd.Categorical()` with `ordered=True` in Python.
...
To visualize the relationship between `likelihood_to_apply` and `GPA`, we will use the following side-by-side plots:
...
# --- Boxplot ---
sns.boxplot(data=college_data, x='decision', y='GPA', hue='decision',
            palette='BuPu', ax=axes[0], legend=False)
axes[0].set_title('Side-by-Side Boxplots', fontsize=19, fontweight='bold')
axes[0].set_xlabel('Decision to Apply to Graduate School', fontsize=18)
axes[0].set_ylabel('GPA', fontsize=17)

# --- Violin Plot with mean point ---
sns.violinplot(data=college_data, x='decision', y='GPA', hue='decision',
               palette='BuPu', ax=axes[1], legend=False)
means = college_data.groupby('decision')['GPA'].mean()
...
axes[1].set_xlabel('Decision to Apply to Graduate School', fontsize=18)
...
prop_summary = (
    college_data.groupby(['field_of_study', 'decision'])
    .size()
    .groupby(level=0)
...
pivot = prop_summary.pivot(index='field_of_study', columns='decision', values='prop').fillna(0)
...
ax.set_xlabel('Parent Education Status', fontsize=18)
ax.set_ylabel('Percent', fontsize=17)
ax.tick_params(axis='x', labelrotation=0, labelsize=17)
ax.tick_params(axis='y', labelsize=17)
ax.legend(title='Decision', fontsize=18, title_fontsize=17,
...
Box and violin plots, along with stacked bar charts, show descriptively how academic performance and parental education background relate to a student's intent to apply to graduate school.

- Students who are `very likely` to apply tend to have the highest GPAs on average, with a median GPA visibly higher than those in the `unlikely` or `somewhat likely` categories.
...
- While there is considerable overlap in the GPA distributions of the `unlikely` and `somewhat likely` groups, the `very likely` group shows a distinct shift toward the higher end of the GPA scale.
- Among students whose parents do not have a graduate degree, a clear majority (59%) report being `unlikely` to apply. In contrast, for students with at least one parent holding a graduate degree, the proportion of those `unlikely` to apply drops to 32%. Notably, the proportion of students who are `very likely` to apply nearly triples—from 8% to 21%—when at least one parent has a graduate degree, indicating that parental academic background may influence a student's likelihood to apply to graduate school.
