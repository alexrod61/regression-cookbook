...
$$
\underline{x}_i = (x_{i,1},...,x_{i,k})
$$ 

In simple terms, $\underline{x}_i$ is just a vector of size $k$ that bundles all $k$ regressors together into a single object, so we can refer to them all at once instead of listing each one.
...
$$
P(Y_i \leq j \mid \underline{x}_i) = p_{i,1} + p_{i,2} + \ldots + p_{i,j} \quad \quad j = 1, 2, \ldots, J-1
$$
...
$$
\begin{aligned}
&\text{logit}(P(Y_i \leq j \mid \underline{x}_i)) = \log\left( \frac{P(Y_i \leq j \mid \underline{x}_i)}{ 1 - P(Y_i \leq j \mid \underline{x}_i)} \right) \\
&= \log\left( \frac{P(Y_i = 1 \mid \underline{x}_i) + P(Y_i = 2 \mid \underline{x}_i) + \ldots + P(Y_i = j \mid \underline{x}_i)}{ P(Y_i = j+1 \mid \underline{x}_i) + P(Y_i = j+2 \mid \underline{x}_i) + \ldots + P(Y_i = J \mid \underline{x}_i) } \right)
\end{aligned}
$$
...
$$
\text{logit}(P(Y_i \le j \mid \underline{x}_i)) = \frac{ P(Y_i \le j \mid \underline{x}_i) }{ P(Y_i \ge j+1 \mid \underline{x}_i) }
$$
...
$$
\text{logit}( P(Y_i \le j \mid \underline{x}_i) ) = \alpha_j + \beta_1 x_{i,1} + \beta_2 x_{i,2} + ... + \beta_k x_{i,k}
$$
...
- $P(Y_i \le 1 \mid \underline{x}_i)$ versus $P(Y_i > 1 \mid \underline{x}_i)$
- $P(Y_i \le 2 \mid \underline{x}_i)$ versus $P(Y_i > 2 \mid \underline{x}_i)$
- $\vdots$
- $P(Y_i \le J-1 \mid \underline{x}_i)$ versus $P(Y_i > J-1 \mid \underline{x}_i)$
...
$$
\text{logit}( P(Y_i \le j \mid \underline{x}_i) ) = \alpha_j + \beta_{1,j} x_{i,1} + \beta_{2,j} x_{i,2} + \ldots + \beta_{k,j} x_{i,k}
$$
...
The ordinal logistic regression model describes the cumulative probabilities $P(Y_i \le j \mid \underline{x}_i)$, that is, the chance of falling in category $j$ or any lower category. To estimate the model, however, we need the category-specific probability $p_{i,j} = P(Y_i = j \mid \underline{x}_i)$, the chance of landing in exactly one category. We recover it by a simple differencing step: the probability of being in category $j$ alone is the probability of being at or below $j$, minus the probability of being at or below the category just beneath it,

$$
p_{i,j} = P(Y_i = j \mid \underline{x}_i) = P(Y_i \le j \mid \underline{x}_i) - P(Y_i \le j-1 \mid \underline{x}_i), \qquad j = 1, \ldots, J.
$$

The two endpoints are fixed by convention: nothing can fall below the first category, so $P(Y_i \le 0 \mid \underline{x}_i) = 0$, and everything falls at or below the last, so $P(Y_i \le J \mid \underline{x}_i) = 1$.

To write this in terms of the model parameters, recall the **logistic cumulative function**

$$
F(\eta) = \frac{e^{\eta}}{1 + e^{\eta}}, \qquad \text{with systematic component} \quad \eta_{i,j} = \alpha_j - \boldsymbol{\beta}^\top \underline{x}_i,
$$

so that $P(Y_i \le j \mid \underline{x}_i) = F(\eta_{i,j})$. Substituting into the differencing formula gives the category probability in closed form,

$$
p_{i,j} = F(\eta_{i,j}) - F(\eta_{i,j-1}) = \frac{e^{\alpha_j - \boldsymbol{\beta}^\top \underline{x}_i}}{1 + e^{\alpha_j - \boldsymbol{\beta}^\top \underline{x}_i}} - \frac{e^{\alpha_{j-1} - \boldsymbol{\beta}^\top \underline{x}_i}}{1 + e^{\alpha_{j-1} - \boldsymbol{\beta}^\top \underline{x}_i}}.
$$
...
$$
\mathcal{L}(\boldsymbol{\alpha}, \boldsymbol{\beta} \mid y) = \prod_{i=1}^n \prod_{j=1}^J p_{i,j}^{y_{i,j}},
$$

where $n$ is the number of individuals in the sample, $J$ is the number of ordered categories, and $p_{i,j} = P(Y_i = j \mid \underline{x}_i)$ is the model-implied probability that individual $i$ (with regressor values $\underline{x}_i$) falls in category $j$, as derived earlier by differencing the cumulative probabilities. The two unknown parameter sets being estimated are collected in $\boldsymbol{\alpha} = (\alpha_1, \alpha_2, \ldots, \alpha_{J-1})^\top$, the vector of thresholds (the category-specific intercepts), and $\boldsymbol{\beta} = (\beta_1, \beta_2, \ldots, \beta_K)^\top$, the vector of $K$ shared slopes that apply across all cumulative splits.
...
$$
\ell(\boldsymbol{\alpha}, \boldsymbol{\beta}) = \sum_{i=1}^n \sum_{j=1}^J y_{i,j} \log p_{i,j}.
$$
...
$$
\frac{\partial \ell}{\partial \alpha_j} = \sum_{i=1}^n \left[ \frac{y_{i,j}}{p_{i,j}} - \frac{y_{i,j+1}}{p_{i,j+1}} \right] f(\alpha_j - \boldsymbol{\beta}^\top \underline{x}_i),
$$

where $f(\cdot) = F(\cdot)(1 - F(\cdot))$ is the logistic density. The partial derivative with respect to each shared slope $\beta_k$ is:

$$
\frac{\partial \ell}{\partial \beta_k} = -\sum_{i=1}^n x_{i,k} \sum_{j=1}^{J} y_{i,j} \left[ \frac{f(\alpha_j - \boldsymbol{\beta}^\top \underline{x}_i)}{p_{i,j}} - \frac{f(\alpha_{j-1} - \boldsymbol{\beta}^\top \underline{x}_i)}{p_{i,j}} \right].
$$
...
2. Repeat until convergence:
   ...
   - Compute cumulative probabilities $P(Y_i \le j \mid \underline{x}_i)$ and category probabilities $p_{i,j}$.
...
$$
\text{logit}(P(Y_i \le j \mid \underline{x}_i)) = \alpha_j - (\beta_1 x_{i,1} + \beta_2 x_{i,2} + \ldots + \beta_K x_{i,K}), \quad j = 1, \ldots, J-1,
$$
...
In the case study model with regressors $x_{i,\texttt{GPA}}$, $x_{i,\texttt{GRE}}$, and the `field-of-study` dummy variables $Z_{i,\texttt{field}}$, the fitted proportional odds model can be written as:

$$
\text{logit}\!\left(P(Y_i \le j \mid \underline{x}_i)\right) = \hat{\alpha}_j - \left(\hat{\beta}_{\texttt{GPA}}\, x_{i,\texttt{GPA}} + \hat{\beta}_{\texttt{GRE}}\, x_{i,\texttt{GRE}} + \hat{\beta}_{\texttt{field}}\, Z_{i,\texttt{field}}\right), \quad j = 1, 2, 3,
$$
...
$$
\text{logit}( P(Y_i \le j \mid \underline{x}_i) ) = \alpha_j - (\beta_{1,j} x_{i,1} + \beta_{2,j} x_{i,2} + \ldots + \beta_{k,j} x_{i,K}), \quad j = 1, \ldots, J-1,
$$
...
$$
\ell(\boldsymbol{\alpha}, \boldsymbol{B}) = \sum_{i=1}^n \sum_{j=1}^J y_{i,j} \log p_{i,j},
$$

but now the category probabilities $p_{i,j}$ are computed from cumulative probabilities that use split-specific slopes $\boldsymbol{\beta}_j = (\beta_{1,j}, \ldots, \beta_{k,j})^\top$:

$$
P(Y_i \le j \mid \underline{x}_i) = \frac{e^{\alpha_j - \boldsymbol{\beta}_j^\top \underline{x}_i}}{1 + e^{\alpha_j - \boldsymbol{\beta}_j^\top \underline{x}_i}}.
$$
...
$$
\text{logit}\!\left(P(Y_i \le j \mid \underline{x}_i)\right) = \hat{\alpha}_j - \hat{\boldsymbol{\beta}}^\top \underline{x}_i.
$$
...
$$
\text{logit}\!\left(P(Y_i \le j \mid \underline{x}_i)\right) = \alpha_j - \left(\beta_{\texttt{GPA}}\, x_{i,\texttt{GPA}} + \beta_{\texttt{GRE},j}\, x_{i,\texttt{GRE}} + \boldsymbol{\beta}_{\texttt{field}}^\top \mathbf{Z}_{i,\texttt{field}}\right), \quad j = 1, 2, 3,
$$
...
The partial proportional odds model, in which `GPA` and `field_of_study` are constrained to shared coefficients while `GRE_score` is allowed cutpoint-specific coefficients, is the model selected for this case study, based on the Brant–Wald test results from the Inference section. Using the sign convention $\text{logit}(P(Y_i \le j \mid \underline{x}_i)) = \alpha_j - \hat{\boldsymbol{\beta}}^\top \underline{x}_i$, the fitted model is:

$$
\text{logit}\!\left(P(Y_i \le j \mid \underline{x}_i)\right) = \hat{\alpha}_j - \left(\hat{\beta}_{\texttt{GPA}}\, x_{i,\texttt{GPA}} + \hat{\beta}_{\texttt{GRE},j}\, x_{i,\texttt{GRE}} + \hat{\beta}_{\texttt{Business}}\, Z_{i,\texttt{Business}} + \hat{\beta}_{\texttt{Hum}}\, Z_{i,\texttt{Hum}} + \hat{\beta}_{\texttt{STEM}}\, Z_{i,\texttt{STEM}} + \hat{\beta}_{\texttt{SocSci}}\, Z_{i,\texttt{SocSci}}\right),
$$
