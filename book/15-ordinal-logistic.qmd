...
On the test set, the model achieves an overall accuracy of **41.2%**, a macro recall of **37.2%**, and a macro F1 of **40.0%**. The confusion matrix shows a clear ordinal pattern in the errors: most misclassifications land in adjacent categories rather than opposite ends of the ordinal scale. For example, the model rarely confuses `Low` with `Very High` or vice versa; mistakes tend to be one step off, which is an important property for an ordinal outcome.
