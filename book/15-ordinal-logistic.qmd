...
$$
\text{logit}\!\left(P(Y_i \le j \mid \mathbf{x}_i)\right) = \hat{\alpha}_j - \left(\hat{\beta}_{\texttt{GPA}}\, x_{i,\texttt{GPA}} + \hat{\beta}_{\texttt{GRE}}\, x_{i,\texttt{GRE}} + \hat{\boldsymbol{\beta}}_{\texttt{field}}^\top \mathbf{Z}_{i,\texttt{field}}\right)
$$
...
On the test set, the model achieves an overall accuracy of **41.2%**, a macro recall of **37.2%**, and a macro F1 of **40.0%**. Rather than showing the tidy near-diagonal pattern, the confusion matrix shows that the model concentrates its predictions at the two ends of the ordinal scale. It labels students either `Low` or `Very High` and never predicts `Medium` at all. For example, 12 students with `Low` inclination of applying to graduate school, are predicted `Very High` and another 12 students with `Very High` inclination are predicted `Low`. Among 49 students with `Medium` inclination of applying to graduate school, the predictions are 21 `Low`, 2 `High`, and 16 `Very High`. The model captures the broad high versus low contrast but resolves the interior of the ordinal scale poorly.
