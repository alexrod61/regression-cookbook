...
$$
\text{logit}(P(Y_i \le j \mid \mathbf{x}_i)) = \frac{ P(Y_i \le j \mid \mathbf{x}_i) }{ P(Y_i \ge j+1 \mid \mathbf{x}_i) }
$$
...
$$
F(\eta) = \frac{e^{\eta}}{1 + e^{\eta}}, \qquad \text{with systematic component} \quad \eta_{i,j} = \alpha_j - \boldsymbol{\beta}^\top \mathbf{x}_i,
$$

so that $P(Y_i \le j \mid \mathbf{x}_i) = F(\eta_{i,j})$. Substituting into the differencing formula gives the category probability in closed form,
...
In the case study model with regressors $x_{i,\texttt{GPA}}$, $x_{i,\texttt{GRE}}$, and the `field-of-study` dummy variables $\mathbf{Z}_{i,\texttt{field}}$, the fitted proportional odds model can be written as:
...
$$
\text{logit}( P(Y_i \le j \mid \mathbf{x}_i) ) = \alpha_j - (\beta_{1,j} x_{i,1} + \beta_{2,j} x_{i,2} + \ldots + \beta_{K,j} x_{i,K}), \quad j = 1, \ldots, J-1,
$$
