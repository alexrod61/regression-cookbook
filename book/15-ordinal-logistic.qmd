...
On the training set, the model achieves an overall accuracy of **52.5%**, a macro recall of **49.4%**, and a macro F1 of **46.7%**. As with any multi-class ordinal problem, these figures should be interpreted relative to the random-chance baseline of 25% for a four-category outcome, so the model meaningfully outperforms chance, but leaves substantial room for improvement.
...
The `Very High` category is the best-predicted class (recall = 0.87, F1 = 0.73). The `High` category is the most difficult to classify correctly (recall = 0.12, F1 = 0.19), as it sits between two better-separated groups and receives predictions pulled in both directions. Macro recall (0.494) is noticeably lower than accuracy (0.525), indicating mild imbalance in how well the model performs across the four categories, a pattern that persists on the held-out test set and is discussed further in the Results section.
