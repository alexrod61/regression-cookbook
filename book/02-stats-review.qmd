...
flavour_bin <- rbinom(n, size = 1, prob = 0.65)
...
# Simulate binary outcomes: 1 = chocolate, 0 = vanilla
flavour_bin = np.random.binomial(n = 1, p = 0.65, size = n)
...
``` {.r}
pi <- round(mean(children_pop$fav_flavour == "chocolate"), 2)
pi
```
...
``` {.python}
pi = round((children_pop["fav_flavour"] == "chocolate").mean(), 2)
print(pi)
```
...
``` {.r}
beta <- round(mean(waiting_pop$waiting_time), 0)
beta
```
...
``` {.python}
mean_waiting_time = round(waiting_pop["waiting_time"].mean(), 0)
