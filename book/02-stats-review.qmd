...
Note that the **demand query** corresponds to the $i$th `r colourize("random variable", "blue")` $D_i$, where the subindex $i$ ranges from $1$ to $n_D$. The term $n_D$ represents the **size of our sample** for this query and theoretically indicates the number of `r colourize("random variables", "blue")` we intend to observe from our `r colourize("population", "blue")` of interest during our sampling. On the other hand, for the **time query**, we have the $j$th `r colourize("random variable", "blue")` $T_j$, with the subindex $j$ ranging from $1$ to $n_T$. In the context of this query, $n_T$ denotes the **size of our respective sample** and indicates how many `r colourize("random variables", "blue")` we plan to observe from our `r colourize("population", "blue")` of interest as part of our sampling.
...
Moving along in this query, recall the $j$th `r colourize("continuous random variable", "blue")` (as in @tbl-queries-3) $T_i$ is distributed as follows:
...
&= \frac{1}{\beta} \int_{t_j = 0}^{t_j = \infty} y \exp \left( -\frac{t_j}{\beta} \right) \mathrm{d}t_j. \\
\end{align*}
