...
Using the above definition, let us retake the PFM from @eq-binomial-PMF and turn into a likelihood function given $n = 10$ trials and $y = 7$ observed successes:
...
To estimate the parameters $\pi$ and $\beta$ using MLE, we can pursue either an **empirical** or **analytical approach** based on our observed data, which is stored in `children_sample` and `waiting_sample`. The following two sections will elaborate on these approaches.
...
In general, the first derivative $f'(y)$ of a function $f(y)$ indicates when the slope of this function is zero, meaning that the corresponding tangent line is flat. A slope of zero indicates the presence of **critical points** in the function $f(y)$, which can either be a maximum or a minimum. In the context of MLE, our goal is to identify the critical point within the range of our population parameter that corresponds to a maximum. Essentially, we aim to find the estimate that makes our observed data, as indicated by the likelihood function, the most plausible (or likely!). Additionally, we need to confirm whether this critical point is indeed a maximum; further discussion in this section will address this by applying the **second derivative test**.
...
3. We set @eq-partial-derivative-log-likelihood-demand to zero and solve for $\pi$. This will yield the close-form solution, and thus the estimate $\hat{\pi}_{\text{MLE}}$:
...
\pi n_d = \sum_{i = 1}^{n_d} d_i \qquad \Rightarrow \qquad \hat{\pi}_{\text{MLE}} = \frac{\sum_{i = 1}^{n_d} d_i}{n_d}.
\end{gather*}
$$

4. Is $\hat{\pi}_{\text{MLE}}$ truly a maximum? Let us apply second derivative test. If $$\frac{\partial^2}{\partial \pi^2} L \left( \pi | d_1, \dots, d_{n_d} \right) \Bigg|_{\pi = \hat{\pi}_{\text{MLE}}} < 0,$$ then $\hat{\pi}_{\text{MLE}}$ is indeed a maximum point:
...
$$
\begin{align*}
\frac{\partial^2}{\partial \pi^2} L \left( \pi | d_1, \dots, d_{n_d} \right) \Bigg|_{\pi = \hat{\pi}_{\text{MLE}}} &= - \frac{\sum_{i = 1}^{n_d} d_i}{\hat{\pi}_{\text{MLE}}^2} - \frac{n_d - \sum_{i = 1}^{n_d} d_i}{\left( 1 - \hat{\pi}_{\text{MLE}} \right)^2} \\
&= - \frac{n_d \hat{\pi}_{\text{MLE}}}{\hat{\pi}_{\text{MLE}}^2} - \frac{n_d - n_d \hat{\pi}_{\text{MLE}}}{\left( 1 - \hat{\pi}_{\text{MLE}} \right)^2} \\
&= - \frac{n_d}{\hat{\pi}_{\text{MLE}}} - \frac{n_d \left( 1 - \hat{\pi}_{\text{MLE}} \right)}{\left( 1 - \hat{\pi}_{\text{MLE}} \right)^2} \\
&= - \frac{n_d}{\hat{\pi}_{\text{MLE}}} - \frac{n_d}{1 - \hat{\pi}_{\text{MLE}}} < 0.
\end{align*}
$$

We pass the second derivative test given that the sample size $n_d$ and the estimate $\hat{\pi}_{\text{MLE}}$ will always be **nonnegative**. Therefore, the estimate

$$
\hat{\pi}_{\text{MLE}} = \frac{\sum_{i = 1}^{n_d} d_i}{n_d},
$${#eq-mle-demand}
...
3. We set @eq-partial-derivative-log-likelihood-time to zero and solve for $\beta$. This will yield the close-form solution, and thus the estimate $\hat{\beta}_{\text{MLE}}$:
...
n_t = \frac{\sum_{j = 1}^{n_t} t_j}{\beta} \qquad \Rightarrow \qquad \hat{\beta}_{\text{MLE}} = \frac{\sum_{j = 1}^{n_t} t_j}{n_t}.
\end{gather*}
$$

4. Is $\hat{\beta}_{\text{MLE}}$ truly a maximum? Let us apply second derivative test. If $$\frac{\partial^2}{\partial \beta^2} L \left( \beta | t_1, \dots, t_{n_t} \right) \Bigg|_{\beta = \hat{\beta}_{\text{MLE}}} < 0,$$ then $\hat{\pi}_{\text{MLE}}$ is indeed a maximum point:
...
$$
\begin{align*}
\frac{\partial^2}{\partial \beta^2} L \left( \beta | t_1, \dots, t_{n_t} \right) \Bigg|_{\beta = \hat{\beta}_{\text{MLE}}} &= \frac{n_t}{\hat{\beta}_{\text{MLE}}^2} - \frac{2 \sum_{j = 1}^{n_t} t_j}{\hat{\beta}_{\text{MLE}}^3} ]\\
&= \frac{n_t}{\hat{\beta}_{\text{MLE}}^2} - \frac{2 n_t \hat{\beta}_{\text{MLE}}}{\hat{\beta}_{\text{MLE}}^3} \\
&= \frac{n_t}{\hat{\beta}_{\text{MLE}}^2} - \frac{2 n_t}{\hat{\beta}_{\text{MLE}}^2} \\
&= -\frac{n_t}{\hat{\beta}_{\text{MLE}}^2} < 0.
\end{align*}
$$

We pass the second derivative test given that the sample size $n_t$ and the estimate $\hat{\pi}_{\text{MLE}}$ will always be **nonnegative**. Therefore, the estimate

$$
\hat{\beta}_{\text{MLE}} = \frac{\sum_{j = 1}^{n_t} t_j}{n_t},
$${#eq-mle-time}
...
Based on the possible numerical values that $d_i$ can take, we can see that $\hat{\pi}_{\text{MLE}}$ is analogous to the mainstream average! Specifically, we are estimating the proportion of children in our population who prefer chocolate over vanilla. This directly addresses both the statement and the parameter we are interested in, which can be referenced in @tbl-queries-3. Therefore, we are ready to numerically compute $\hat{\pi}_{\text{MLE}}$ by using the corresponding mean function from `R` and `Python` via `children_sample`.
...
where $n_t$ denotes the sample size of waiting times, and $t_j$ in minutes is a continuous observation that falls within the range $[0, \infty)$. This estimate is analogous to the calculation of the mainstream average. Additionally, $\hat{\beta}_{\text{MLE}}$ enables us to estimate the average waiting time between customers at any given ice cream cart within our population of interest, as indicated by the statement and parameter discussed by @tbl-queries-3. That said, we can now proceed to calculate $\hat{\beta}_{\text{MLE}}$ numerically using the appropriate mean function in both `R` and `Python` using `waiting_sample`.
...
::::
::::{.Heads-up-container}
The MLE invariance property states that if we already an estimate of a parameter, such as $\hat{\pi}_{\text{MLE}}$ in a Bernoulli-distributed population, then any monotonic transformation of parameter $\pi$ will yield a corresponding estimate. For example, consider the transformation:
...
$$
\begin{align*}
g(\hat{\pi}_{\text{MLE}}) &= \hat{\pi}_{\text{MLE}} (1 - \hat{\pi}_{\text{MLE}}) \\
&= \hat{\text{Var}}_{\text{MLE}}(D_i).
\end{align*}
$$
::::
:::
...
That said, the above MLE paradigm will significantly influence our upcoming chapters on regression modeling. MLE is a key aspect that makes parameter estimation in many regression models, based on sample data, a transparent process rather than a black box. This paradigm allows data scientists to perform analyses on data derived from specific populations by employing a generative modelling approach. We assume that this data originates from particular distributions, which have their corresponding parameters to estimate. 

Furthermore, as we will see later in this book, these parameters will not only pertain to measures of central tendency and uncertainty but will also include regression parameters. Of course, the MLE approaches in these models will be more complex, as we will need to estimate multiple parameters simultaneously. Still, the frequentist concept of producing the estimates that make our observed data the "*most plausible (or likely!)*" will remain central to our analyses. Essentially, this framework underpins the model fitting functions in both `R` and `Python`, which we will explore later in most chapters.
::::
:::
