...
The `r colourize("parameter", "blue")` definition in @imp-parameter points out a crucial fact in investigating any given `r colourize("population", "blue")` or system: 
...
Note the definition of the `r colourize("probability", "blue")` for an event $A$ in @imp-probability specifically highlights the following:
...
Under the umnbrella of this approach, we assume that our governing `r colourize("parameters", "blue")` are **fixed**. Note that, within the philosophy of this school of thinking, we can only make **precise** and **accurate** predictions as long as we repeat our $n$ experiments as many times as possible, i.e., 
