...
- For the **time query**, we are interested in the `r colourize("parameter", "blue")` $\beta$, which represents the average waiting time in minutes from one customer to the next one in our `r colourize("population", "blue")` of interest. Unlike the above $\pi$ `r colourize("parameter", "blue")`, $\beta$ is only positively unbounded given the definition of our random variable $T_j$. Therefore, this definition establishes our time query `r colourize("parameter", "blue")` as follows:
...
For the sake of this book, we will not explore more than one `r colourize("measure of uncertainty", "blue")`. Specifically, we refer to the `r colourize("variance", "blue")` (and its derived measure called **standard deviation**) given that it is commonly used across the different regression models in subsequent chapters.
