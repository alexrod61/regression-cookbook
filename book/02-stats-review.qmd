...
::::
::::{.definition-container}
Let $Y$ be a `r colourize("discrete", "blue")` or `r colourize("continuous random variable", "blue")` whose support is $\mathcal{Y}$. Furthermore, let us assume that the mean of $Y$ is

$$
\mathbb{E}(Y) = \mu.
$$

Then, the `r colourize("variance", "blue")` of $Y$ is the `r colourize("mean", "blue")` of the squared deviation from the corresponding `r colourize("mean", "blue")` as follows:

$$
\begin{align*}
\text{Var}(Y) &= \mathbb{E}\left\{[ Y - \mathbb{E}(Y)]^2 \right\} \\
&= \mathbb{E}\left[ (Y - \mu)^2 \right].
\end{align*}
$${#eq-first-variance}
...
$$
\begin{align*}
\text{Var}(Y) &= \mathbb{E} \left[ \left( Y - \mu \right)^2 \right] \\
&= \mathbb{E} \left( Y^2 - 2Y \mu + \mu^2 \right) \\
&= \mathbb{E}(Y^2) - \mathbb{E} \left( 2 Y \mu \right) + \mathbb{E} \left( \mu \right)^2 \\
& \qquad \text{distributing the expected value operator}\\
&= \mathbb{E}(Y^2) - 2 \mathbb{E} \left( Y \mu \right) + \mathbb{E} \left( \mu \right)^2 \\
& \qquad \text{since $2$ is a constant}\\
&= \mathbb{E}(Y^2) - 2 \mu \mathbb{E} \left( Y \right) + \mu^2 \\
& \qquad \text{since $\mu$ is a constant}\\
&= \mathbb{E}(Y^2) - 2 \mu^2 + \mu^2 \\
&= \mathbb{E}(Y^2) - \mu^2.  \qquad \qquad \qquad \qquad \qquad \square
\end{align*}
$$
:::::
::::
