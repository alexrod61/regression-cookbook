...
&= \sum_{i} \sum_{j \in I_i} z_i P_Y(Y = y_j) \\
&= \sum_{i} \sum_{j \in I_i} g(y_j) P_Y(Y = y_j).
\end{align*}
...
$$
\begin{align*}
\mathbb{E}(Z) &= \sum_{i} \sum_{j \in I_i} g(y_j) P_Y(Y = y_j) \\
&= \sum_{y \in \mathcal{Y}} g(y) P_Y(Y = y) \\
&= \mathbb{E}\left[ g(Y) \right].  \qquad \qquad \qquad \square
\end{align*}
$$
::::::
:::::
...
::::
::::{.Heads-up-container}
After applying some algebraic rearrangements and `r colourize("expected value", "blue")` properties, @eq-first-variance and @eq-second-variance are equivalent as follows:
...
::::
::::{.definition-container}
A `r  colourize("generative model", "blue")` is
::::
:::
