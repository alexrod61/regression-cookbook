...
::::
::::{.Heads-up-container}
Although we are assigning a value of $\pi = 0.65$ as our true `r colourize("population parameter", "blue")` in this query, we can never know the exact value in practice unless we conduct a full census. This is why we rely on probabilistic tools, via random sampling and statistical inference, to estimate this $\pi$.
::::
:::

Let us recall that we are assuming each child is modelled as a Bernoulli trial, where a success (denoted as `1`) indicates that the child "*prefers chocolate*." This also reflects the flavour mapping in the code. Furthermore, instead of using a Bernoulli random number generator, we are utilizing a [**Binomial**](#sec-binomial-distribution) random number generator. This is because the Binomial case with `r colourize("parameters", "blue")` $n = 1$ and $\pi$ is equivalent to a Bernoulli trial with `r colourize("parameter", "blue")` $pi$. Hence, in general, consider the following general Binomial case:
...
The final output of this quick simulation, which models a population of children as Bernoulli trials with a `r colourize("probability", "blue")` of success $\pi = 0.65$, consists of a data frame containing $N_D = 2,000,000$ rows, with each row representing a child and their preferred ice cream flavor: either **chocolate** or **vanilla**. It is worth noting that the outputs from both `R` and `Python` differ due to the fact that each language employs **different pseudo-random number generators**.
...
In terms of the executive meeting with the eight general managers, it would not be an efficient use of time to go individually over these $n_D = 500$ and $n_T = 1000$ data points along with abstract mathematical concepts such as `r colourize("PMFs", "blue")` or `r colourize("PDFs", "blue")`, as well as probabilistic definitions of `r colourize("random variables", "blue")` and `r colourize("parameters", "blue")` represented by Greek letters. Instead, there should be a more straightforward and simple way to explain how these `r colourize("random variables", "blue")` behaved during our data collection process. The key to addressing this complexity lies in understanding `r colourize("measures of central tendency", "blue")` and `r colourize("uncertainty", "blue")`.
