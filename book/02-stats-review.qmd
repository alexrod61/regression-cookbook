...
Let us recall the **mainstream average** from @eq-mainstream-average is considered an `r colourize("estimate", "blue")`. Now, moving along with our key concepts, it is time to have a crucial conversation on the difference between **likelihood** and `r colourize("probability", "blue")`. 
...
To make our explanation more graphical, we will plot the `r colourize("PMFs", "blue")` of six Binomial `r colourize("random variables", "blue")` as in @fig-binomial-PMFs.
...
plot_grid(binomial_PMF(n = 10, pi = 0.3, interest = 5) +
  ...
  ggtitle(expression(paste("Binl(n = 10, ", pi, " = 0.6)"))),
