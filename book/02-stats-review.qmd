...
bernoulli_sd_plot <- ggplot2::ggplot() +
  ...
  ggplot2::geom_vline(xintercept = c(0.5, 0.7, 0.9), colour = "darkorange1", linetype = "longdash") +
...
exponential_sd_plot <- ggplot2::ggplot() +
  ...
  ggplot2::labs(
    x = expression(mu * " (minutes)"),
    y = expression(SD(T[j]) * " in minutes")
  ) +
...
ggplot(
  ...
  labs(
    x = "Sample Mean Waiting Time (minutes)",
    y = "Absolute \nFrequency "
  ) +
...
ggplot(
  ...
  labs(
    x = "Number of chocolate preferences out of 10 children",
    y = expression(paste(p[Y], "(y; n, ", pi, ")"))
  )
```
...
plot_binomial_likelihood_pi <- ggplot(data = data.frame(x = c(0, 1)), aes(x)) +
  ...
  labs(
    x = expression(pi),
    y = expression(paste("L(", pi, "; ", y[obs], " = 7, n = 10)"))
  ) +
  theme_bw() +
  theme(
    axis.text = element_text(size = 15.5),
