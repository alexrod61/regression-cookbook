...
**Statistical notation** can be pretty particular and different from **usual mathematical notation**. One of these particularities is the constant use of **Greek letters** to denote unknown `r colourize("population parameters", "magenta")` in **modelling setup**, **estimation**, and **statistical inference**. In that spirit, throughout this book, we use diverse Greek letters to denote our regression `r colourize("parameters", "magenta")` across each of the outlined models in every chapter.
...
During early learning stages of regression modelling, we may feel overwhelmed by these new letters, which could be unfamiliar. Therefore, whenever confusion arises in any of the main chapters in this book regarding the names of these letters, we recommend checking out the Greek alphabet from table @tbl-greek-alphabet. Note that 
`r colourize("frequentist", "magenta")` statistical inference mostly uses lowercase letters. **Finally, with practice over time, you would likely end up memorizing most of this alphabet.**
