...
In case the reader is not 100% familiar with probabilistic and inferential topics, as discussed above, I will provide a fundamental refresher in @sec-intro with crucial points that are needed to follow along the **statistical way** each one of the chapters is delivered (more specifically for **modelling estimation/training** matters!).\
