...
This book mainly focuses on statistical **regression analysis** with connections to **supervised learning**. Thus, it is not introductory statistics and machine learning material. Also, some coding background on `R` [@r2024] and/or `Python` [@python] is recommended. That said, the following topics are suggested:
...
- **Foundations of linear algebra.** Some chapters of this cookbook use matrix-based notation and linear algebra ideas, particularly when introducing feature vectors, design matrices, and model estimation. A review of *vectors*, *matrices*, and *matrix multiplication* can therefore be useful. The [MDS learning resources page](https://ubc-mds.github.io/resources_pages/learning_resources/) includes several relevant options, such as the highly recommended.
