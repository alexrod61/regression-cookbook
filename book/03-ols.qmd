...
Each student in this dataset was asked for their current grade out of 100, how many previous programming courses they previously took, and how much they scored on the final exam out of 30, all stored as variables `current_grade`, `prior_courses`, and `exam_score` respectively. 
...
### A. Exploratory Data Analysis {.unnumbered}
...
# Read data
df <- read_csv("./problem-1-data.csv")
...
df = pd.read_csv("./problem-1-data.csv") 
...
### B. Initial Modelling {.unnumbered}
...
### C. Model Improvements {.unnumbered}
1. Should `prior_courses` be converted to another variable type? Try converting it to categorical and re-fit. Do you notice any improvements?
...
## R Code
```{r}
# C.1 Convert prior_courses to categorical and fit model
...
```{python}
# C.1 Convert prior_courses to categorical
