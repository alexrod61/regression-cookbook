...
Note that the index $i$ presents the $i$-th individual in the sample and the $j$-th presents the $j$-th regressor. The model can be extended to include interaction terms between regressors but we will not consider that in this example.
...
The goal of estimation is to determine the best-fitting regression line by minimizing the sum of squared residuals—the differences between the observed and predicted values. This step provides a mathematical basis for analyzing financial behaviors in students. Although functions such as `lm()` estimate regression coefficients automatically (as we will see in the next section), understanding the mathematical foundation of ordinary least squares helps clarify how linear regression models are fit. The following tip briefly reviews the underlying theory.
...
Here, the superscript ${}^\top$ denotes the matrix transpose operation, which converts a column vector into a row vector. This allows the expression to produce a single scalar value representing the residual sum of squares.
