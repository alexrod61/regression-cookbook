...
Let us load this sample of $n = 1,000$ houses in both `R` and `Python`. For `Python`, we will need the [{pandas}](https://pypi.org/project/pandas/) library. @tbl-toy-data-r and @tbl-toy-data-py show the first 100 rows of this full dataset and `R` and `Python`, respectively.
...
- **Simple Linear Regression** models the relationship between a single regressor and the response variable. This approach is suitable when we suspect a dominant factor driving financial balance.
...
Together, these predictors provide a comprehensive representation of the economic, behavioral, and lifestyle factors that may help explain variation in student financial balance.
...
More generally, a multiple linear regression model with $p$ regressors can be written as

$$
Y_i = \beta_0 + \sum_{j=1}^{p} \beta_j x_{i,j} + \epsilon_i.
$$

The model can extend including interaction terms between regressors but we will not consider that in this example.
