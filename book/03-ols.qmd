...
|---|---|---|
| **Simple Linear Regression** | Models the relationship between one predictor variable and a continuous response variable. This approach is useful when the goal is to examine whether a single dominant factor explains variation in financial balance. | @montgomery2012 |
...
| **Polynomial Regression** | Extends linear regression by adding polynomial terms, such as squared or cubic predictors, to capture curved relationships between predictors and the response variable. | @montgomery2012 |
...
Given that our goal is to examine how multiple factors-such as income, expenses, and living arrangements—affect students’ financial balance, we select **Multiple Linear Regression** via OLS. This method allows us to quantify the influence of each regressor while controlling for confounding effects.

### Defining the Regression Model

Once we select OLS regression, we define the response variable and the set of predictor variables used to explain variation in the response and write the regression equation. Suppose we observe data from $n$ students. Let $Y_i$ represent the continuous financial balance (`Net_Money`) in CAD for the $i$-th student, where $i = 1, 2, \ldots, n$.
...
This objective function gives a mathematical meaning to the phrase “best-fitting line.” The best-fitting model is the one whose coefficients make the squared residuals as small as possible.

Although functions such as `lm()` in R and `ols()` in Python estimate regression coefficients automatically, it is helpful to understand the mathematical foundation behind ordinary least squares. The following tip briefly shows how the same least squares idea can be written compactly using matrix notation and how the coefficients are estimated so that the sum of squared of residuals is minimized. 
...
In practice, software such as `lm()` in R computes these estimates automatically using efficient numerical algorithms rather than explicitly calculating matrix inverses. Although we do not need to perform these calculations by hand, it is still important to understand what happens behind the scenes when fitting a linear regression model using the `lm()` function.
...
\hat{\beta}_9x_{i,9}
+
\hat{\beta}_{10}x_{i,10}
+
\hat{\beta}_{11}x_{i,11}
+
\hat{\beta}_{12}x_{i,12}.
\end{align*}
$$
...
Similarly, $x_{i,8}$ represents `Monthly_Earnings`. The estimated coefficient for this regressor is $0.92$. Holding the other regressors fixed, a one-dollar increase in monthly earnings is associated with an estimated increase of 0.92 CAD in predicted financial balance, on average.

The interpretation of binary regressors is slightly different. For example, $x_{i,1}$ represents `Has_Job`, where $x_{i,1}=1$ if the student has a job and $x_{i,1}=0$ otherwise. The estimated coefficient is $91.24$. Therefore, holding the other regressors fixed, students with a job are estimated to have a financial balance that is 91.24 CAD higher, on average, than students without a job.

The coefficient of $x_{i,3}$ is $-488.10$. Since $x_{i,3}$ represents `Financially_Dependent`, this estimate compares students who are financially dependent to students who are not financially dependent. Holding the other regressors fixed, financially dependent students are estimated to have a financial balance that is 488.10 CAD lower, on average, than students who are not financially dependent.

In this model, the ordinal variables `Year_of_Study` and `Goes_Out_Spends_Money` are treated as numeric regressors. Therefore, their coefficients are interpreted as linear one-unit changes. For example, $x_{i,2}$ represents `Year_of_Study`, and its estimated coefficient is $-100.14$. This means that, holding the other regressors fixed, moving up by one academic year is associated with an estimated decrease of 100.14 CAD in predicted financial balance, on average. Similarly, $x_{i,6}$ represents `Goes_Out_Spends_Money`, and its estimated coefficient is $-47.85$. This means that a one-unit increase on the spending-frequency scale is associated with an estimated decrease of 47.85 CAD in predicted financial balance, on average, holding the other regressors fixed.
...
```{r}
#| label: fig-residuals-vs-fitted-net-money
...
# Plot
residuals_vs_fitted <- ggplot(aug_training, aes(x = .fitted, y = .resid)) +
  geom_point(alpha = 0.55, color = "#0073C2FF", size = 2) +
...
```{r}
#| label: fig-residuals-index-net-money
...
```{r}
#| label: fig-cooks-distance-net-money
