...
When we set a `r colourize("random variable", "blue")` $Y$, we also set a new set of $v$ possible `r colourize("outcomes", "magenta")` $\mathcal{Y} = \{ y_1, \dots, y_v\}$ coming from the `r colourize("sample space", "blue")` $S$. This new set of possible `r colourize("outcomes", "magenta")` $\mathcal{Y}$ corresponds to the support of the `r colourize("random variable", "blue")` $Y$ (i.e., all the possible values that could be taken on once we execute a given random experiment involving $Y$). 
...
Then, the `r colourize("probability distribution", "blue")` of $Y$, i.e., $P_Y(\cdot)$ assigns a `r colourize("probability", "blue")` to each **observed value** $Y = y_j$ (with $j = 1, \dots, v$) if and only if the `r colourize("outcome", "magenta")` of the random experiment belongs to the `r colourize("sample space", "blue")`, i.e., $s_i \in S$ (for $i = 1, \dots, u$) such that $Y(s_i) = y_j$:
...
A `r colourize("probability model", "blue")` is a mathematical representation of how a `r colourize("random variable", "blue")` behaves under uncertainty in a `r colourize("population", "blue")` or system of interest. It specifies the possible values the `r colourize("random variable", "blue")` can take together with a distribution that describes how probable those values are, usually through one or more governing `r colourize("parameters", "blue")`. This idea aligns with the notion of a `r colourize("generative model", "blue")`, where we use a `r colourize("probabiity distribution", "blue")` to describe how the observed data could have been generated from the `r colourize("population", "blue")` or system under study.
...
Let $A$ be an event of interest in a random phenomenon in a `r colourize("population", "blue")` or system of interest. The `r colourize("sample space", "blue")` $S$ of event $A$ denotes the set of all the possible **random `r colourize("outcomes", "magenta")`** we might encounter every time we randomly observe $A$ such as we were running some class of experiment.

Note each of these `r colourize("outcomes", "magenta")` has a determined `r colourize("probability", "blue")` associated with them. If we add up all these `r colourize("probabilities", "blue")`, the `r colourize("probability", "blue")` of the sample $S$ will be one, i.e.,
