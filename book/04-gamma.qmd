...
This plot shows how the shape parameter changes the look of the Gamma distribution. It helps explain why Gamma regression is such a good match for skewed data.

## How Gamma Regression Works

Gamma regression helps us understand how the average value of something positive changes with different conditions. But it does this in a slightly different way than regular linear regression.

> Instead of predicting the outcome directly, Gamma regression predicts the log of the average outcome. This is done using something called a **link function**.
...
A link function is just a transformation that helps the model do a better job. For Gamma regression, the most common link function is the log link.
::::
:::

Using the log has three main benefits:

- It helps control large numbers by flattening them out
- It makes sure our predictions stay positive
- It turns certain patterns into straight lines, which are easier to model

This means that Gamma regression is actually fitting a model like:
...
Later, we can convert the predicted log value back to the original scale using the exponential function:

$$
\mu_i = \exp(\beta_0 + \beta_1 x_{i1} + \cdots + \beta_p x_{ip})
$$

This guarantees that our predictions will always be positive.
...
The idea of a **link function** can seem abstract at first, but a quick plot can help make it more concrete. In Gamma regression, we usually use the **log link**, which means the model fits a straight line to the log of the mean response, then transforms it back to the original scale.
...
- In Gamma regression, we fit a model to the log of the expected outcome $\log(\mu)$, not directly to $\mu$.
- The left plot shows a straight-line relationship: $\log(\mu) = 2 + 0.5x$.
- The right plot shows the back-transformed version: $\mu = \exp(2 + 0.5x)$, which curves upward.
- This transformation ensures that all predicted values are positive and allows the model to capture multiplicative effects that are common in skewed, positive data.
...
Gamma regression is often used when we want to understand how average outcomes change under different conditions. It is especially helpful when those outcomes vary widely between observations.
...
In all these cases, we are interested in typical outcomes, not just predicting individual values. Gamma regression lets us model these averages in a flexible and meaningful way.

## When to Use and Not Use Gamma Regression

Now that we understand what Gamma regression does and how it models positive, skewed outcomes, it is helpful to summarize the specific conditions under which this method is appropriate and when it might not be a good fit.
