...
In this case study, the multinomial logistic regression model helps us understand how commute distance, age, car availability, and weekend commuting status relate to an individual's primary mode of transport, an outcome with four unordered categories: `Car`, `Public_Transit`, `Bicycle`, and `Walking`. Below, we distill the key takeaways from the analysis and discuss what they imply for commuters, transportation planners, and policy makers.

### Key Insights From the Model

Car availability and commute distance are the two most consistently significant regressors across both the imbalanced and upsampled models. Having access to a car is associated with markedly lower odds of using public transit or walking instead of driving; the corresponding effect for cycling is not statistically distinguishable from zero, so car access alone does not appear to explain the choice to bike in this dataset. Commute distance moves the odds of public transit and cycling in opposite directions relative to driving: each additional kilometre increases the odds of choosing public transit over the car, while decreasing the odds of cycling over the car. This matches intuition — the car and public transit are the practical options once a commute gets long, while cycling stays attractive mainly over short distances.

Age and weekend-commuting status tell a more conditional story. On the original, imbalanced training data, neither regressor reaches statistical significance for most categories, largely because the sample contains too few cyclists and walkers to estimate their effects precisely. Once the minority classes are upsampled, both regressors become significant predictors of `Walking`, and age additionally becomes significant for `Bicycle`: older individuals and weekend commuters are more likely to walk than drive, once the class imbalance no longer masks the effect. This is a useful reminder that failing to reject $H_0$ is not the same as confirming $H_0$ — for rare categories, a lack of significance can reflect insufficient data rather than a genuinely null effect.
