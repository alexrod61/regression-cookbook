...
The residual deviance is smaller than the null deviance by $238.17$ units. This reduction is captured by the likelihood-ratio statistic $G^2 = 238.17$ on 12 degrees of freedom ($p < 0.001$), providing strong evidence that a model with age, commute distance, car availability, and weekend commuting status fits the training data better than the intercept-only model. McFadden's $R^2 = 0.165$ is still below the 0.2–0.4 range commonly associated with a good fit for discrete-response models, which indicates that these four regressors together explain only a modest share of the variation in `transport_mode` on the log-likelihood scale, even though their joint contribution is highly significant. The AIC ($1236.60$) and BIC ($1308.64$) reported here describe the imbalanced training model; they are most useful later for comparing this fit against the upsampled model or other model specifications.
...
Whether people choose driving or taking public transits varys significantly on average. The commute distance and the availability of cars both have significant effects on the choices of transportation mode. Note that the R output flags the `weekend_commuterYes` coefficient for `Bicycle` as significant, with an odds ratio and standard error of essentially zero and an extreme test statistic; this is a spurious artifact of complete separation, since no `Bicycle` riders in the test set are weekend commuters, so the coefficient is not identifiable. `statsmodels` instead assigns this term a very large standard error, yielding a large p-value, so it is correctly excluded from the significant coefficients in the Python output.
...
Age and weekend-commuting status tell a more conditional story. On the original, imbalanced training data, neither regressor reaches statistical significance for most categories, largely because the sample contains too few cyclists and walkers to estimate their effects precisely. Once the minority classes are upsampled, both regressors become significant predictors of `Walking` (and, to a lesser extent, `Bicycle`): older individuals and weekend commuters are more likely to walk than drive, once the class imbalance no longer masks the effect. This is a useful reminder that failing to reject $H_0$ is not the same as confirming $H_0$ — for rare categories, a lack of significance can reflect insufficient data rather than a genuinely null effect.

One estimate needs a caveat rather than a substantive interpretation. On the test set, the `weekend_commuter` effect for `Bicycle` comes back with an enormous coefficient, an enormous standard error, and a $p$-value near 1. This is not evidence of no effect; it is a symptom of complete separation, since none of the bicycle commuters in the test set commute on weekends, so the term is not identifiable from the available data. Analysts should flag this kind of estimate rather than interpret it, and should be cautious of near-zero or implausibly large odds ratios whenever a category is small.
