...
Why does the chapter use the Softmax form to convert the logit-linear functions into category probabilities?
::::
:::
...
The Softmax form converts the $m - 1$ baseline-category logits into probabilities for all $m$ categories. It ensures that every predicted probability is between 0 and 1 and that the probabilities across the categories sum to 1 for each observation.
...
The chapter notes that changing the baseline category, for example from `Car` to `Public_Transit`, does not change the estimated probability for each category. If the probabilities do not change, what does change?
::::
:::
...
Changing the baseline changes the **parameterization** of the model: the intercepts, coefficients, and log-odds comparisons are rewritten relative to a different reference category. The fitted category probabilities remain the same because they describe the same modelled distribution, only expressed using a different baseline.
...
You can subtract the two baseline-category logit equations. For example, the log odds comparing `Walking` to `Bicycle` can be obtained by taking the `Walking` versus `Car` linear predictor and subtracting the `Bicycle` versus `Car` linear predictor, which also means subtracting the corresponding coefficients.
...
Downsampling reduces the number of observations in the majority class so the classes are more balanced, but it can throw away useful data. Upsampling increases the number of minority-class observations, often by resampling with replacement, but it can increase the risk of overfitting to repeated minority cases.
...
Suppose an exponentiated 95% confidence interval for an odds ratio is $(0.82, 1.24)$. At the 5% significance level, would this provide strong evidence that the predictor changes the odds for that category comparison? Why or why not?
::::
:::
...
No. Because the confidence interval includes 1, the data are compatible with no multiplicative change in the odds for that category comparison at the 5% significance level. This would usually correspond to a p-value greater than 0.05 for the associated coefficient.
...
The chapter distinguishes an inferential objective from a predictive objective. In the transportation case study, what is one example of each?
::::
:::
...
An inferential objective is to estimate how predictors such as `commute_distance`, `age`, `has_car_available`, and `weekend_commuter` are associated with the odds or probabilities of different transport modes. A predictive objective is to classify a new individual's most likely `transport_mode` using those same predictors and then evaluate performance with tools such as a confusion matrix, accuracy, macro recall, and macro F1.
...
## **`Python` Code**
```{python}
sns.countplot(data=train_set, x="lunch_choice")
...
sns.boxplot(data=train_set, x="lunch_choice", y="weekly_food_budget")
plt.xlabel("Lunch choice")
...
meal_props[levels].plot(kind="bar", stacked=True)
plt.xlabel("Has meal plan")
plt.ylabel("Proportion")
plt.legend(title="Lunch choice", bbox_to_anchor=(1.02, 1), loc="upper left")
plt.show()
...
time_props[levels].plot(marker="o")
