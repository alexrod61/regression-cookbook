...
&= \beta_0 + \beta_1 X_{\mathrm{width}} \\
\log \left( \lambda_{\mathrm{L}} \right) &= \beta_0 + \beta_1 X_{\mathrm{width}} + \beta_2 X_{\mathrm{color_darker}_{\mathrm{L}}} + \beta_3 X_{\mathrm{color_light}_{\mathrm{L}}} + \beta_4 X_{\mathrm{color_medium}_{\mathrm{L}}} \\
&= \beta_0 + \beta_1 X_{\mathrm{width}}+ \beta_2 \times 0 + \beta_3 \times 1 + \beta_4 \times 0 \\
&= \beta_0 + \underbrace{\beta_1 X_{\mathrm{width}}}_{\text{Constant}} + \beta_3.
\end{align*}
$$
...
$$
\begin{align*}
\log \left( \frac{\lambda_{\mathrm{D}} }{\lambda_{\mathrm{L}}} \right) &= \log \left( \lambda_{\mathrm{L}} \right) - \log \left( \lambda_{\mathrm{D}} \right) \\
&= \beta_4.
\end{align*}
$$
...
Let's now predict on the test set. The model performance is measured by Kullback-Leibler (KL) divergence, which is defined as
$$
...
Averaging $\bar{D}_{\mathrm{KL}}(y\Vert \hat{\lambda}) = \frac{1}{n} \sum_{i=1}^n D_{\mathrm{KL}}(\hat{y_i} \Vert \hat{\lambda}_i)$ across all observations gives a measure of how well the model captures the true data distribution. Smaller averaged KL values indicate better fit.
...
Poisson deviance is also a standard, interpretable metric to measure the goodness of fit of Poisson regression. It is likelihood-based and equals to 2 times KL divergence summed across all points, i.e.,
$$\text{Poisson Deviance = } 2\sum_i D_{\mathrm{KL}}(y_i\Vert \hat{\lambda}_i).$$
It measures the difference in log-likelihood between a saturated model (perfect fit) and the fitted model. Lower deviance values indicate the model predictions are closer to the observed data.
