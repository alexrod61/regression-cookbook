...
The general idea to solve the Poisson regression in @eq:pois-k is to find the parameters $\lambda_j$s by maximizing the following likelihood: 
$$
...
Since the objective in @eq:pois-log is not quadratic and there is no closed form solution, we could not solve the equation exactly. Iteratively reweighted least squares (IRLS) is a method to find an approximated solution with a specified accuracy or tolerance. The basic idea is that IRLS uses Newton-Raphson or Fisher scoring to solve an approximated problem of the original objective @eq:pois-log and solve it iteratively with weights updated through iteration until the specified accuracy is achieved, i.e., the objective is converged. 

The approximated problem of the original objective at iterate $t+1$ is a weighted linear least squares problem written as: 
$$
  \begin{align}
    \boldsymbol{\beta}^{t+1} &= {\arg\min}_{\boldsymbol{\beta}} \sum_{i=1}^n w_i^t (y_i - X_i^{\top}\boldsymbol{\beta})^2 \\ 
    &= {\arg\min}_{\boldsymbol{\beta}} \sum_{i=1}^n w_i^t (y_i - \beta_0 - \beta_1 x_{i,\text{width}})^2 \label{eq:irls-approx}
  \end{align}
$$
given the estimates at time $t$ for the univariate case using `crabs` dataset. The approximated problem has a closed-form solution and is much easier to solve than the original objective. 
The weights comes from the second derivative (Hessian) of the log-likelihood. In our case, the weights at iterate $t+1$ can be updated given the coefficient estimates at $t$ using the following formula:
$$
