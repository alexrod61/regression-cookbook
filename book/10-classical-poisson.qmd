...
This chapter introduces a generalized linear regression model that can be applied on count data that is Poisson distributed. Compared to ordinary regression models, which assumes normality and constant variance--homoskedasticity, Poisson regression models count data which is skewed and heteroskedastic. 
...
- How can the number of complaints received by a telecom company from customers be explained by service type and contract length?
- How does the distribution of counts of satellite male crabs residing around a female crab nest vary by the biological characteristics of the nests?
...
Below is a graphical demonstration for the comparison of linear regression model with Gaussian (normal) distributed responses with Poisson regression model with Poisson distributed response. The linear regression has a linear fitted blue line (in the left panel), while the Poisson regression, due to the use of log link function, has a fitted blue curve (in the right panel). Segment the explanatory variable (in the x axis) in each scenario into five sections using the gray dashed vertical lines. The red horizontal segments represent the frequencies of binned response in each section, which represents the rotated histogram of the response. In linear regression, the response in each section is symmetrically distributed with similar variation across sections (i.e., homoskedasticity); while in Poisson regression, the response is skewed with heteroskedasticity (specifically, increasing variances as the responses increase) across sections. 
...
# scatter plot of satellite size vs width
crabs %>%
...
Let's explore the relationship between the satellite size per nest (`satell`) and width of the female horseshoe crab (`width` in cm) in the following scatterplot.
