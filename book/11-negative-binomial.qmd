...
$$
Y_i \mid x_{i,1}, x_{i,2}, \ldots, x_{i,k} \sim \operatorname{Negative Binomial}(\mu_i,\theta),
...
Let's imagine a series of independent Bernoulli trials. A Negative Binomial distribution is commonly parameterized as $Z\sim\operatorname{Negative Binomial}(\theta,p)$, where $Z$ is the number of failed Bernoulli trials before experiencing $\theta$ independent wins and $p$ is the probability of success in each Bernoulli trial. The mean of $Z$ is given by

$$
\mu=\mathbb{E}(Z)=\frac{\theta(1-p)}{ p}
$$
Rearranging the equation for conditional mean and isolating for $p$ gives

$$
p=\frac{1}{1+\frac{\mu}{\theta}}
$$
For a constant value of $\theta$, there is a one-to-one correspondence between $p$ and $\mu$ and therefore our Negative Binomial model can be parameterized by $\mu_i$ and $\theta$, where $\mu_i$ is the conditional expected number of satellite males for the $i$th female crab.
...
The link function connects the conditional mean $\mu_i$ to the systematic component $\eta_i$. Like [Classical Poisson regression](@sec-classical-poisson), the standard link function in Negative Binomial regression is the **log link**:
...
Let's revisit the `extended_poisson_model` fit in [Chapter 10](#sec-classical-poisson). This model showed substantial overdispersion relative to the Classical Poisson mean-variance assumption with a Pearson dispersion estimate of $3.082$ and a $p-$value < 0.001. We will test whether there is overdispersion in this Poisson regression model via function `dispersiontest()`, (from package `{AER}`). Note that `statsmodels` does not ship an equivalent function, so the `Python` code defines the same regression-based test as a small helper of the same name.
...
As indicated in @sec-negative-binomial-log-link, in Classical Poisson regression, the conditional mean for the $i$th observation is
...
$$
Y_i \mid \mathbf{x}_i \sim \operatorname{Negative Binomial}(\mu_i,\theta),
$$
...
$$
Y_i \mid x_{i,1} \sim \operatorname{Negative Binomial}(\mu_i,\theta),
$$
...
Therefore, a Negative Binomial model assumes **overdispersion**. Once width is included, the model assumes that the conditional variance of the satellite count is higher than the conditional mean for crabs with the same width value. This is stronger than merely saying that the raw sample variance should be higher than the raw sample mean. It is a model statement about the distribution of counts **after conditioning on width**.
