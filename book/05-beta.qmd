...
::::
::::{.Heads-up-container}
If you pay close attention, you'll notice that the R output has a **phi coefficient** and the Python output has a **precision coefficient**.

Recall from our earlier discussion that Beta regression can be expressed in terms of two quantities:

1. the average proportion, denoted by $\mu$, and
2. the precision parameter, denoted by $\phi$, which describes how tightly observations cluster around the average proportion.

The regression coefficients shown above correspond to the mean model, describing how the predictors affect $\mu$. The phi (or precision) coefficient corresponds to the second component of the model and quantifies the amount of variability remaining around the fitted mean.

The two software packages report this parameter differently:

- **R (`betareg`)** reports $\phi$ directly.
- **Python (`statsmodels`)** reports $log(\phi)$, so the value must be exponentiated to obtain $\phi$.

For example, the Python output reports a precision coefficient of 3.7837. Exponentiating this value gives

$$
\phi = e^{3.7837} \approx 43.98,
$$

which matches the phi coefficient reported by R.

Larger values of $\phi$ indicate that observations are more tightly clustered around the predicted mean, while smaller values indicate greater variability in the response.
::::
:::
...
Recall that the model was fit using Beta regression with a logit link. As a result, coefficients describe changes in the log-odds of the mean grade proportion, rather than direct changes in grades themselves. Exponentiating a coefficient yields an odds ratio that describes how the odds of the mean grade proportion i.e., μ/(1−μ) change multiplicatively as a predictor increases. Unlike in logistic regression, this is not the odds of a binary event such as passing or failing; it describes the expected proportion itself, holding all other variables constant.
