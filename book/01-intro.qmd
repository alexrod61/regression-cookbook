...
Let us start by preparing for all the different regression techniques introduced in @sec-ols. We want to highlight one guiding principle for all of our work:
...
This principle rests on foundational statistical ideas on how data is generated and how it can be modelled through various regression methods. We will explore these underlying concepts in @sec-stats-review. Before doing so, however, this chapter will build on the three core pillars introduced in [Audience and Scope](https://alexrod61.github.io/regression-cookbook/book/audience-scope.html): 
...
::::
::::{.Warning-container}
Each data science problem invovling regression analysis has unique characteristics, depending on if the inquiry is inferential (statistics!) or predictive (machine learning!). Different types of outcomes (or response variables) require distinct modelling approaches. For example, we might analyze **survival times** (e.g., the time until one particular equipment of a given brand fails), **categorical outcomes** (e.g., a preferred musical genre in the Canadian young population), or **count-based outcomes** (e.g., how many customers we would expect on a regular Monday morning in the branches of a major national bank), etc. Moreover, under this regression context, our analysis extends beyond the oucome itself, but we also examine how it relates to other variables (**the so-called features!**). For instance, if studying musical genre preferences among young Canadians, we could explore how age groups influence these preferences or compare genre popularity across provinces. **The sky is the limit here!**
