...
Let's start by preparing for all the different regression techniques introduced in @sec-ols. We want to highlight one guiding principle for all of our work:
...
This principle rests on foundational statistical ideas on how data is generated and how it can be modeled through various regression methods. We’ll explore these underlying concepts in @sec-stats-review. Before doing so, however, this chapter will build on the three core pillars introduced in **Audience and Scope**: 

A) Implementing a structured **data science workflow** as outlined in @sec-ds-workflow,

B) Selecting the appropriate workflow approach based on an **inferential** or **predictive** paradigm, as shown in @fig-ds-workflow, and

C) Choosing the **appropriate regression model** based on the response variable or outcome of interest, using the mind map in @sec-regression-mindmap (analogous to a **regression toolbox**).
...
::::
::::{.Warning-container}
Each data science problem invovling regression analysis has unique characteristics, depending on if the inquiry is inferential (statistics!) or predictive (machine learning!). Different types of outcomes (or response variables) require distinct modeling approaches. For example, we might analyze **survival times** (e.g., the time until one particular equipment of a given brand fails), **categorical outcomes** (e.g., a preferred musical genre in the Canadian young population), or **count-based outcomes** (e.g., how many customers we would expect on a regular Monday morning in the branches of a major national bank), etc. Moreover, under this regression context, our analysis extends beyond the oucomt itself, but we also examine how it relates to other variables (the so-called features!). For instance, if studying musical centre preferences among young Canadians, we could explore how age groups influence preferences or compare genre popularity across provinces. **The sky is the limit here!**\

At first glance, it might seem that every regression problem should have a unique workflow tailored to its specific model. **However, this is not entirely the case**. In @fig-ds-workflow, we introduce a structured regression workflow designed as a proof of concept for thirteen different regression models. Each flow is covered in a seperate chapter of this blook alongside a review of probability and statistics (i.e, thirteen chapters in this book aside from the probability and statistics review). This workflow standardizes our approach, making analysis more transparent and efficient while allowing us to communicate insights effectively through data storytelling. Naturally, this workflow includes decision points that determine whether the approach follows an  **inferential** or **predictive** path (the second pillar). As for our third pillar, this comes into play at the data modeling stage, where the regression toolbox @fig-regression-mindmap guides model selection based on the response varaible type.
::::
:::

Let’s establish a convention for using admonitions throughout this textbook. These admonitions will help distinguish between key concepts, important insights, and supplementary material, ensuring clarity as we explore different regression techniques. We will start using these admonitions in section 1.1 @sec-ml-stats-dictionary.

<<<<<<< HEAD
::: {.definition}
=======
::: {#Definition-sample .definition}
>>>>>>> f604739b80c15c089f818f909e2018c0c7a7651d
::::{.definition-header}
Definition
::::
...
::: {.Tip}
=======
An idea (or ideas!) related to a modeling approach, a specific workflow stage, or an important data science concept. This admonition is used to flag crucial statistical or machine learning topics that warrant deeper exploration.
::::
:::
...
<<<<<<< HEAD
Finally, @sec-dictionary will be the section in this book where the reader can find all those `r colourize("statistical", "blue")` and `r colourize("machine learning-related", "magenta")` terms in alphabetical order. Notable terms (either statistical or machine learning-related) will include an admonition identifying which terms (again, either statistical or machine learning-related) are **equivalent** or **somewhat equivalent** (or **even NOT equivalent if that is the case!**). Take as an example the statistical term `r colourize("dependent variable", "blue")`:
=======
In @sec-dictionary, readers will find all those `r colourize("statistical", "blue")` and `r colourize("machine learning-related", "magenta")` terms in alphabetical order. Notable terms will include clear notes on their equivalence or non-equivalence. For instance, consider the statistical term `r colourize("dependent variable", "blue")`:
>>>>>>> cfa81e3ff3cb8839c0171557d643ac58d7a6356d
...
Above, we have identified four equivalent terms for the term `r colourize("dependent variable", "blue")`. Furthermore, these terms can be `r colourize("statistical", "blue")` or `r colourize("machine learning-related", "magenta")`. Finally, note we will start using this colour scheme in @sec-stats-review.
...
::::
::::{.Heads-up-container}
Throughout this book, we will use specific terms interchangeably while explaining different regression methods. If confusion arises, readers should check definitions and equivalences (or non-equivalences) in @sec-dictionary.
::::
:::
