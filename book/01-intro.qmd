...
Note that the nature of the aforementioned **time query** is more related to a larger `r colourize("population", "magenta")`. Therefore, we can define it as **all our ice cream customers during the Summer weekends**. Furthermore, this second definition would limit this query to our corresponding general ice cream customers, given the requirements of our operations team, and **not** all the children between 4 and 11 years old attending the parks during Summer weekends. Consequently, it is crucial to note that the nature of our queries will dictate how we define our `r colourize("population", "magenta")` and our subsequent data modelling and statistical inference.

Summer time represents the most profitable season from a business perspective, thus solving these above two queries is a significant priority for our company. Hence, you decide to organize a meeting with your eight general managers (one per Canadian city). Finally, during the meeting with the general managers, it was decided to do the following:

1. For the **demand query**, a comprehensive market study will be run on the `r colourize("population", "magenta")` of interest across the eight Canadian cities right before next Summer; suppose we are currently in Spring.
...
Bingo! *Ottawa's general manager* just nailed the probabilistic way of making inference on our `r colourize("population parameter", "magenta")` of interest for the **time query**. Indeed, their comment was primarily framed from a business perspective of optimizing operational costs. Still, this fact does not take away a crucial insight on which statistical inference is built: a `r colourize("random sample", "magenta")` (as in @imp-random-sample). As for *Vancouver's general manager*, ironically, their statement is **NOT PRECISE** at all! Mimicking a census-type study might not be the most optimal decision for the **demand query** given the time constraint and the potential size of its target `r colourize("population", "magenta")`. 

> **Realistically, there is no cheap and efficient and cheap way to conduct a census-type study for any of the two queries!**

We can state that `r colourize("probability", "magenta")` is viewed as the language to decode random phenomena that occur in any given `r colourize("population", "magenta")` or system of interest. In our example, we have two random phenomena:
...
Hence, let us finally define what we mean by `r colourize("probability", "magenta")` along with the inherent concept of `r colourize("sample space", "magenta")`.

![Mexican lotería, a bingo-type game. Image by [*irvin Macfarland*](https://unsplash.com/@irvinmac) via [*Unsplash*](https://unsplash.com/photos/blue-white-and-red-playing-cards-clelay10tfg).](img/loteria.jpg){width="550"}
...
::: {#imp-sample-space .callout-important}
...
Note the definition of the `r colourize("probability", "magenta")` for an event $A$ in @imp-probability specifically highlights the following:

>**... as the $n$ times we observe the random phenomenon goes to infinity.**

The "*infinity*" term is key when it comes to an understanding the philosophy behind the `r colourize("frequentist", "magenta")` school of statistical thinking in contrast to its `r colourize("Bayesian", "magenta")`  counterpart. In general, the `r colourize("frequentist", "magenta")` way of practicing statistics in terms of `r colourize("probability", "magenta")` and inference is the approach we usually learn in introductory courses, more specifically when it comes to `r colourize("hypothesis testing", "magenta")` and `r colourize("confidence intervals", "magenta")` which will be explored in @sec-basics-inf. That said, the Bayesian approach is another way of practicing statistical inference. Its philosophy differs in what information is used to infer our `r colourize("population parameters", "magenta")` of interest. Below, we briefly define both schools of thinking.

::: {#imp-frequentist-stats .callout-important}
# Definition of frequentist statisitcs
...
![Image by [*Mari Carmen Díaz*](https://pixabay.com/users/maricarmennd9-3511650/?utm_source=link-attribution&utm_medium=referral&utm_campaign=image&utm_content=1737624) via [*Pixabay*](https://pixabay.com//?utm_source=link-attribution&utm_medium=referral&utm_campaign=image&utm_content=1737624).](img/infinity.jpg){width="400"}
:::

::: {#imp-bayesian-stats .callout-important}
# Definition of Bayesian statisitcs
...
![The unique known portrait of Reverend Thomas Bayes found in @odonnell1936, even though @bellhouse2004 argues it might not be a Bayes' portrait.](img/thomas-bayes.jpg){width="300"}
