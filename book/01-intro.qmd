...
2. Defining Modeling Parameters

After selecting a suitable regression model, the next step is to define the modeling parameters—these are the coefficients that quantify the relationships between the predictor variables and the response variable:

- **Identifying Predictor Variables**: Choose relevant predictors based on EDA insights.
- **Setting Initial Parameter Values**: Define which coefficients will be estimated in the model.
- **Considering Interaction Terms**: Include interaction terms if the effect of one predictor depends on another.

3. Setting Up the Modeling Equation

The modeling equation represents the mathematical relationship between the response variable and the predictor variables, incorporating the estimated coefficients. This equation formalizes how the predictors combine to determine the response, accounting for the influence of each predictor and any interaction effects.

#### Example: Modelling for Housing Price Predictions

To illustrate these steps, let's apply them to a practical example: predicting house prices. This example will show how to use multiple linear regression to model the relationship between house prices and various factors identified during EDA.

1. **Choosing a Suitable Regression Model**

During EDA, suppose we discovered that house prices are influenced by several factors, including square footage, number of bedrooms, and neighborhood quality. We might also notice some non-linear relationships, such as diminishing returns on price increases as square footage becomes very large.

Based on these findings, a **multiple linear regression model** is suitable to capture the combined effect of these variables. If non-linear patterns are strong, we might also consider a **polynomial regression** to capture those nuances.

2. Defining Modelling Parameters

Next, we define the modeling parameters—these are the coefficients representing the impact of each predictor on house prices. Here are some examples of predictor variables:

- Square footage ($X_1$)
- Number of bedrooms ($X_2$)
- Age of the house ($X_3$)
- Proximity to schools ($X_4$)
- Neighborhood quality ($X_5$)

Each of these predictor variables is represented as $X$ with a subscript. In addition to these individual predictors, we may also consider **interaction terms** to capture more complex relationships. For instance, the interaction between square footage and neighborhood quality could reveal how these factors jointly influence the price.

3. Setting Up the Modeling Equation

With our model and parameters defined, we set up the modeling equation for the multiple linear regression model. The equation is:
...
For those who may not be familiar with these symbols, the equation can be interpreted as:
$$
\begin{align}
\text{Price} = & \ \beta_0 \\
...
& + \beta_3 \times \text{Age} \\
& + \beta_4 \times \text{Proximity to School} \\
& + \beta_5 \times \text{Neighborhood Quality} \\
& + \epsilon
\end{align}
$$

The $\text{Price}$ is the response variable (house price), dependent on the five predictor variables defined earlier. The price is also influenced by the $\beta$ coefficients, which are the parameters we aim to estimate. Specifically:

- $\beta_0$ is the intercept, representing the expected base price of a house when all predictors are zero.
- $\beta_1, \beta_2, \beta_3, \beta_4, \beta_5$ are the coefficients for each predictor variable, indicating the effect of each predictor on the house price.

Finally, the error term $\epsilon$ accounts for the variability in house prices that is not explained by the predictors.

If the EDA indicated non-linear relationships, a polynomial regression might be more appropriate. For example:
$$
\begin{align}
\text{Price} = & \ \beta_0 \\
& + \beta_1 \times \text{Square Footage} \\
& + \beta_2 \times \text{Square Footage}^2 \\
& + \beta_3 \times \text{Bedrooms} \\
& + \cdots \\
& + \epsilon
\end{align}
$$
This polynomial regression equation captures non-linear effects, providing a more flexible model for predicting house prices.

By separating the theoretical steps of choosing a model, defining parameters, and setting up the equation, and then applying them to a specific example like house price prediction, we create a clear and structured approach to data modeling. This systematic approach, as visualized in @fig-ds-workflow-data-modelling, lays a solid foundation for understanding how to develop regression models based on data insights.
...
In this stage, we use functions available in R or Python to fit the regression model to the training set. The goal is to estimate the coefficients ($\beta$ values) that best describe the relationship between the predictors and the response variable in the data.

- In R, we can use the `lm()` function for linear regression. This function fits the linear model specified by the formula and returns the estimated coefficients.

```R
# Example in R
model <- lm(Price ~ Square_Footage + Bedrooms + Age + Proximity_to_Schools + Neighborhood_Quality, data = training_set)
```

- In Python, the `statsmodels` library provides a similar function called `OLS` for ordinary least squares regression. Alternatively, the `scikit-learn` library's `LinearRegression` class can be used for fitting linear models.
...
X = training_set[['Square_Footage', 'Bedrooms', 'Age', 'Proximity_to_Schools', 'Neighborhood_Quality']]
X = sm.add_constant(X)
...
These functions take the training set as input and apply the specified regression model to the data. The output includes parameter estimates that describe how each predictor variable influences the response variable.

#### Delivering Parameter Estimates

Once the model is fitted, the estimates for $\beta_0, \beta_1, \cdots, \beta_n$ are available. These estimates allow us to interpret the impact of each predictor on the response variable. The intercept $\beta_0$ represents the expected value of the response when all predictors are set to zero, while each $\beta_i$ indicates the change in the response associated with a one-unit change in the corresponding predictor.

- In R, we can view these coefficients using the `summary()` function:

```R
# Example in R: Outputting parameter estimates
summary(model)
```

- In Python, we can view these coefficients using the `summary()` method from the `statsmodels` library:

```python
# Example in Python: Outputting parameter estimates
print(model.summary())
```

These estimates provide a direct interpretation of the relationship between the predictors and the response variable, based on the training data.
...
After estimating the model parameters, the next critical step in the data science workflow is assessing the **goodness of fit**. This step involves performing model diagnostics to determine how well the estimated model fits the data and whether it satisfies the underlying assumptions of the regression analysis. A well-fitting model not only provides reliable parameter estimates but also ensures that the predictions are meaningful and robust.

As visualized in @fig-ds-workflow-goodness-of-fit, this stage is essential to validate the model before interpreting results. If the model fails to meet the required assumptions, we may need to revisit the data modeling step and make necessary adjustments. Conversely, if the model passes the diagnostics, we can confidently proceed to the next step of interpreting the results.

#### Example: Goodness of Fit for Housing Price Predictions

Let’s use our house price prediction example to see how we can check the goodness of fit. This means we'll see if our model's predictions are accurate and if it makes sense given the data.

Note: In this section, we’ll be using some technical terms and concepts related to model assumptions and fit. It’s perfectly normal if you’re not familiar with all of them right now. These are standard checks that help improve the reliability of the model, and you’ll get more comfortable with them as you practice.

1. Checking Model Assumptions

After estimating our model (for example, using multiple linear regression to predict house prices), we need to check if it follows some basic rules. These rules help ensure our model's predictions are reliable:

- **Linearity**: This means the relationship between house prices and the factors we use to predict them (like square footage) should look like a straight line. To check this, we can make a plot of the predicted prices vs. the actual prices. If the points are scattered around a straight line, it means the linearity assumption is met, and our model is on the right track.
- **Independence**: This assumption means that each house’s price in our sample is independent of the others—one house's price doesn’t affect another’s. If we know the data was collected in a way that ensures this, we can safely move on.
- **Constant Variance**: Also called homoscedasticity, this means the spread of errors (the differences between predicted and actual prices) should be about the same across all predictions. We can check this by plotting the errors (residuals) against the predicted prices. If the spread is even, then the constant variance assumption holds.
- **Normality of Residuals**: This means the errors (residuals) in our predictions should follow a normal distribution, like a bell curve. We can use a Q-Q plot (a type of graph) to check this. If the points form a straight line in the plot, the normality assumption is likely met.

2. Evaluating Model Fit

We can also check how well our model fits the data using some numbers:

- **R-squared**: This number shows how much of the changes in house prices our model can explain. For example, an R-squared of 0.75 means our model explains 75% of the variability in house prices, which is a good fit.
- **Adjusted R-squared**: This is similar to R-squared but adjusted for the number of predictors (like square footage, number of bedrooms). It helps prevent overfitting, which happens when a model fits the training data too closely but doesn't perform well on new data.
- **F-statistic**: This number helps us understand if our model is useful overall. A significant F-statistic suggests that our predictors (like square footage and number of bedrooms) are explaining the house prices better than just guessing.

3. Identifying Outliers and Influential Points

Sometimes, certain data points can heavily affect our model:

- **Outliers**: These are prices that are much different from what our model predicts. Outliers can be detected by looking at the residuals (the differences between actual and predicted prices). If a price is very far from what was predicted, it might be an outlier.
- **Influential Points**: Some data points can have a big effect on the model’s results. Tools like Cook’s distance help us find which points might be influencing the model too much. Points with high Cook’s distance are those we need to look at closely.

4. Taking Action if Needed

If our checks show problems, we can take action to improve the model:

- **Transform Variables**: If the data is skewed or doesn’t meet assumptions, we can transform variables, such as taking the log of prices, to help meet the assumptions.
- **Adjust the Model**: We might add interaction terms to see how different factors together affect house prices (e.g., how the number of bedrooms affects price differently in different neighborhoods).
- **Try a Different Model**: If standard linear regression doesn’t fit well, we might use another model, like a generalized linear model (GLM), especially if the residuals are not normal.
