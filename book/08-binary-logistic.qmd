...
# --- 1) Wald tests (in summary output) ---
summary(model_full)

# --- 2) Odds ratios + 95% CI ---
# Option A (profile likelihood CI; default in confint)
or_ci <- exp(cbind(OR = coef(model_full), confint(model_full)))
or_ci

# Option B (Wald-style CI using standard errors; matches summary more closely)
or_ci_wald <- exp(cbind(OR = coef(model_full), confint.default(model_full)))
or_ci_wald
...
Once we’ve established that predictors matter, the next step is to interpret **what the coefficients mean**.

Logistic regression coefficients live on the **log-odds** scale. That scale is convenient for estimation, but not very intuitive—so we usually interpret effects using **odds ratios** and **predicted probabilities**.
...
# Add predicted probabilities + predicted class (do NOT overwrite BLR)
pred_tbl <- BLR %>%
  mutate(
...
conf_mat(pred_tbl, truth, predicted)

metric_set(accuracy, sens, spec, precision, f_meas)(pred_tbl, truth, predicted)

# ROC curve + AUC
roc_df  <- roc_curve(pred_tbl, truth, pred_prob)
roc_auc(pred_tbl, truth, pred_prob)
autoplot(roc_df)
```
...
pred_tbl %>% roc_curve(truth, pred_prob)
```
