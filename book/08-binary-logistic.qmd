...
# The `colourize` function applies text color formatting depending on output format.
# - LaTeX/PDF: wraps text in \textcolor{}
# - HTML: wraps text in a <span> with inline CSS color
# - Other (e.g., Word): returns text unchanged
...
# - Share the dataset with Python via reticulate
...
library(lmtest)        # Tests for linear/nonlinear models
library(reticulate)    # R ↔ Python interoperability
library(broom)         # Tidy model outputs
library(rsample)       # Train/test splitting (tidymodels)
library(yardstick)     # Classification metrics (tidymodels)
library(pscl)          # Pseudo R-squared for logistic regression
library(pROC)          # ROC curves and AUC
library(arm)           # Binned residual plots
...
# ---- Python dependencies (must run before Python initializes) ----
...
# ---- Reproducibility ----
set.seed(1234)
reticulate::py_run_string("import numpy as np; np.random.seed(1234)")
...
## Why OLS Fails and the Logit Solution
...
The key idea is to **transform the probability** so that it can be modeled as a linear function without breaking these rules — and the **logit function** does exactly that.
...
- `age` (numeric)
- `education_years` (count; years of schooling)
- `married` (categorical: 0/1)
...
fit_glm <- glm(defaulted ~ credit_score + income + age + education_years + married,
               data = BLR,
               family = binomial)
...
df = r.BLR.copy()
X = df[['credit_score', 'income', 'age', 'education_years', 'married']]
y = df['defaulted']
...
Let's focus on a simplified model using just two key variables: `credit_score` and `income`.
...
-   **Original:** `married = "Yes"`
-   **Encoded:** `married_yes = 1`

**Note on the “Dummy Variable Trap” (Intercept + both levels).**  
If `married` has two categories (Yes/No), a common encoding is to create two indicators: `married_yes` and `married_no`. However, if your model includes an **intercept** (which is the default in both R and Python), you **must not** include indicators for *all* categories. Why? Because for every row,
$$
\text{married\_yes} + \text{married\_no} = 1,
$$
...
In practice, we avoid this by choosing a **reference level** and dropping one indicator. The effect of the reference level is absorbed into the **intercept**, and the remaining coefficient(s) are interpreted *relative to that reference group*. For example, if “No” is the reference, then the coefficient on `married_yes` compares married individuals to non-married individuals, holding other predictors fixed.
...
# drop_first=True prevents the "Dummy Variable Trap" (e.g., keeps 'married_Yes', drops 'married_No')
df_clean = pd.get_dummies(df_clean, columns=['married', 'owns_home'], drop_first=True)
...
Because each outcome $Y_i$ is Bernoulli with success probability $\pi_i$, least squares is not the natural fitting criterion. Instead, we estimate $\boldsymbol\beta$ using **maximum likelihood**.

Assuming the observations are conditionally independent given the predictors, the likelihood of the observed data is

$$
L(\boldsymbol\beta)=\prod_{i=1}^n \pi_i^{y_i}(1-\pi_i)^{1-y_i},
$$

where $\pi_i$ is linked to the predictors through the logistic model

$$
\pi_i=\frac{1}{1+\exp\!\left(-(\beta_0+\beta_1X_{1i}+\cdots+\beta_kX_{ki})\right)}.
$$

Plugging this expression for $\pi_i$ into $L(\boldsymbol\beta)$ gives a likelihood written entirely in terms of $\boldsymbol\beta$. It is usually easier to work with the **log-likelihood**:
...
The **maximum likelihood estimate** $\hat{\boldsymbol\beta}$ is the value of $\boldsymbol\beta$ that maximizes $\ell(\boldsymbol\beta)$.

::: {.callout-note}
In practice, software computes $\hat{\boldsymbol\beta}$ using an iterative method. In R, `glm(..., family = binomial)` fits logistic regression via **IRLS/IWLS** (Iteratively Reweighted Least Squares), which repeatedly solves a weighted least-squares problem until the coefficients converge. (This procedure is closely related to Newton-style optimization of the log-likelihood, but you don’t need those details to use the model.)
:::
...
logit_model <- model_full  # alias for downstream chunks
...
# ===================================
# 2) ODDS RATIOS WITH 95% CI
# ===================================

# Table version using broom (Wald-style CI based on standard errors)
coef_table <- tidy(model_full, conf.int = TRUE, conf.level = 0.95) %>%
  mutate(
...
knitr::kable(
  ...
  caption = "Logistic regression coefficients with odds ratios (Wald 95% CI)"
)

# If you prefer profile likelihood CI, use:
...
# Optional: same LR test via lmtest
...
or_table = pd.DataFrame({
    "Log-Odds (β)": params,
    "OR": np.exp(params),
    "OR 2.5%": np.exp(conf["Lower"]),
    "OR 97.5%": np.exp(conf["Upper"]),
...
df_diff = int(model_full.df_model - model_simple.df_model)
p_value = chi2.sf(LR_stat, df_diff)

print("\nLikelihood Ratio Test: simple vs full")
print(f"LR = {LR_stat:.4f}, df = {df_diff}, p = {p_value:.4g}")
```
:::

### Interpreting the results in our example

From the model summary, each predictor has an estimated coefficient $\hat\beta_j$ (log-odds scale), a standard error, and a Wald p-value. The odds-ratio table converts the same information to a more interpretable scale via $\exp(\hat\beta_j)$.

To interpret a coefficient, always state the comparison **holding the other predictors fixed**. For example:

- If the odds ratio for `credit_score` is below 1, then higher credit scores are associated with *lower* odds of default.
- For `income`, it is usually best to rescale (e.g., per \$10,000) so the odds ratio corresponds to a meaningful change.

Finally, the likelihood ratio test compares the two models directly: if the LR p-value is small, the additional predictor(s) provide evidence of improved fit beyond the simpler model.
...
$$
\log\!\left(\frac{\pi_i}{1-\pi_i}\right)=\beta_0+\beta_1X_{1i}+\cdots+\beta_kX_{ki},
$$

then a one-unit increase in \(X_j\) (holding all other predictors fixed) changes the log-odds by \(\beta_j\). Exponentiating converts that change into an **odds ratio**:

$$
\text{OR}_j = e^{\beta_j}.
$$

- If \(\text{OR}_j = 1\), there is no association.
- If \(\text{OR}_j < 1\), higher values of \(X_j\) are associated with **lower odds** of the outcome.
- If \(\text{OR}_j > 1\), higher values of \(X_j\) are associated with **higher odds** of the outcome.

For changes larger than one unit, the odds ratio scales multiplicatively:

$$
\text{OR for a } \Delta\text{-unit increase} = e^{\beta_j \Delta}.
$$

This is why it’s often better to interpret effects using meaningful units (e.g., *50 credit-score points* or *\$10,000 of income*) rather than “per 1 unit”.

### Pitfalls in interpretation

Odds ratios describe changes in **odds**, not changes in **probability**. The same odds ratio can correspond to a large probability change in one part of the predictor range and a small change in another.

So, when communicating results, a good pattern is:

1) report the odds ratio (direction + multiplicative size), and  
2) translate it into predicted probabilities for a concrete scenario (e.g., a borrower with credit score 600 vs 700, holding income fixed).

### Example: translating odds ratios into predicted probabilities

In the code below, we compute odds ratios for meaningful changes (e.g., +50 credit-score points, +\$10,000 income) and then compute predicted probabilities for two borrower profiles that differ only in credit score.
...
# Odds ratios (per 1 unit) with 95% CI
# If you want Wald-style CI (matches summary SEs), use confint.default()
or_ci <- exp(cbind(OR = coef(logit_model), confint.default(logit_model)))
or_ci

# Odds ratios for meaningful changes
b_cs <- coef(logit_model)["credit_score"]
b_inc <- coef(logit_model)["income"]

OR_50pts  <- exp(b_cs * 50)
OR_10kinc <- exp(b_inc * 10000)

OR_50pts
OR_10kinc

# Example: predicted probability at credit_score = 600 vs 700
# holding income fixed at its mean
new_data <- data.frame(
  credit_score = c(600, 700),
...
# Odds ratios for meaningful changes
b_cs  = params["credit_score"]
b_inc = params["income"]

OR_50pts  = np.exp(b_cs * 50)
OR_10kinc = np.exp(b_inc * 10000)

print("\nOR for +50 credit-score points:", OR_50pts)
print("OR for +$10,000 income:", OR_10kinc)

# Example: predicted probability at credit_score = 600 vs 700 (income fixed at mean)
test_data = pd.DataFrame({
    "const": 1,
    "credit_score": [600, 700],
    "income": [df["income"].mean(), df["income"].mean()]
})
print("\nPredicted probabilities:", model_full.predict(test_data).values)
```
:::
...
#| fig-cap: "Predicted probability of default by credit score (simple logistic model)."
#| fig-width: 8
#| fig-height: 5
...
credit_seq <- seq(min(BLR$credit_score, na.rm=TRUE),
                  max(BLR$credit_score, na.rm=TRUE),
                  length.out = 200)

pred_df <- data.frame(credit_score = credit_seq)

# predictions + approximate 95% CI via linear predictor SE
...
# choose reference scores that are inside the observed range
ref_scores <- quantile(BLR$credit_score, probs = c(0.1, 0.5, 0.9), na.rm=TRUE)
ref_df <- data.frame(credit_score = as.numeric(ref_scores))
ref_df$prob <- predict(model_simple, newdata = ref_df, type = "response")

# ensure defaulted is numeric 0/1 for plotting points
y_points <- as.integer(as.character(BLR$defaulted))

ggplot(pred_df, aes(x = credit_score, y = prob)) +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.2) +
  geom_line(linewidth = 1.2) +
  geom_jitter(data = BLR,
              aes(x = credit_score, y = y_points),
              height = 0.03, alpha = 0.15, size = 0.8) +
  geom_point(data = ref_df, aes(x = credit_score, y = prob), size = 2.5) +
...
# CI on logit scale -> prob scale
pred_logit = model_simple.predict(pred_df, linear=True)
X_pred = pred_df[["const", "credit_score"]].values
vcov = model_simple.cov_params().values
...
fig, ax = plt.subplots(figsize=(8, 5))
ax.fill_between(credit_seq, lower_ci, upper_ci, alpha=0.2, label="95% CI")
ax.plot(credit_seq, pred_probs, linewidth=2, label="Predicted probability")

# light jitter for points
rng = np.random.default_rng(42)
jitter = rng.normal(0, 0.02, size=len(df))
ax.scatter(df["credit_score"], df["defaulted"] + jitter, alpha=0.15, s=10)

# reference scores at quantiles (always in range)
ref_scores = np.quantile(df["credit_score"], [0.1, 0.5, 0.9])
ref_df = pd.DataFrame({"const": 1, "credit_score": ref_scores})
ref_probs = model_simple.predict(ref_df)

ax.scatter(ref_scores, ref_probs, s=50)
ax.set_xlabel("Credit Score")
ax.set_ylabel("Predicted Probability of Default")
ax.set_ylim(0, 1)
...
print("Reference points (credit score -> predicted probability):")
for s, p in zip(ref_scores, ref_probs):
    print(f"{s:.0f} -> {p:.3f}")
...
**Interpretation:** The curve is steepest in the middle of the credit-score range, meaning the same improvement in credit score produces a larger probability change there than at the extremes.

To make this concrete, the code prints predicted probabilities at a few credit-score reference points (chosen from the observed distribution), which you can report directly without hard-coding numbers into the text.
...
pred_tbl <- BLR %>%
  mutate(
    ...
    pred_class = if_else(pred_prob >= threshold, 1, 0),
    truth      = factor(defaulted, levels = c(0, 1)),
    predicted  = factor(pred_class, levels = c(0, 1))
  ) %>%
  select(truth, predicted, pred_prob)

# Confusion matrix
pred_tbl %>% conf_mat(truth, predicted)

# Metrics (note the ~ syntax)
pred_tbl %>%
  metric_set(accuracy, sens, spec, precision, f_meas)(truth = truth, estimate = predicted)

# ROC + AUC (uses probabilities)
pred_tbl %>% roc_auc(truth, pred_prob)
autoplot(pred_tbl %>% roc_curve(truth, pred_prob))
```
...
# Threshold (change to 0.3 to see trade-off)
threshold = 0.5
...
print(classification_report(y, df["pred_class"], digits=3))
...
<!-- TODO: Add plot of deviance residuals vs. predicted probabilities. -->
<!-- TODO: Add binned residual plot for calibration. -->
<!-- TODO: Add influence plot (highlighting high-leverage or influential cases). -->
<!-- TODO: Provide a short interpretation using the loan default dataset. -->
...
### Storytelling: Connecting Statistics to Decisions
...
## When to Use, Limitations, and Extensions
...
Logistic regression assumes that the relationship between each continuous predictor and the log-odds of the outcome is linear. This means the model expects a consistent, directional shift in log-odds for every one-unit increase in the predictor — regardless of where on the scale you are. In reality, many relationships are nonlinear.

In our loan default dataset, for example, we assumed that credit score affects log-odds linearly across the full range of 300 to 850. But it is entirely plausible that risk drops sharply at low credit scores and then levels off at higher ones — a pattern a linear term cannot capture. If this nonlinearity is present and ignored, the model will systematically mispredict at the extremes.

You can check this assumption by plotting smoothed observed default rates (e.g., using a lowess smoother) against the predictor. If the relationship looks curved, consider adding polynomial terms such as `credit_score + I(credit_score^2)`, using regression splines, or fitting a Generalized Additive Model (GAM), which estimates the shape of the relationship from the data rather than imposing it in advance.

#### 2. Sensitive to Outliers and Influential Points

Because logistic regression is estimated by maximum likelihood rather than least squares, it is not immune to the influence of unusual observations. A single borrower with an extreme combination of predictor values — say, a very low credit score but unusually high income — can pull coefficient estimates away from what would best represent the majority of the data.

This is why model diagnostics matter. Cook's distance and leverage statistics (covered in the Diagnostics section) help identify cases that exert disproportionate influence. When you find such cases, the right response is not necessarily to delete them — sometimes they are the most interesting observations — but to understand why they are unusual and report whether removing them changes your conclusions substantially.

#### 3. Requires Sufficient Sample Size

Maximum likelihood estimation works by finding the parameter values that make the observed data most probable. With small samples, the likelihood surface becomes flat and unreliable — estimates can be unstable, standard errors wide, and in extreme cases the algorithm may fail to converge at all. This problem is especially acute when the outcome is rare: a dataset with 500 observations but only 10 defaults gives very little information for estimating which predictors drive default risk.

A commonly cited rule of thumb is at least 10 to 15 events per predictor variable in the model. If your data falls short of this, consider Firth's penalized likelihood method, which adds a small penalty that stabilizes estimates in small samples and is available in R via the `logistf` package.

#### 4. Cannot Handle Perfect Separation

Perfect separation occurs when some predictor (or combination of predictors) perfectly predicts the outcome — for instance, if every borrower with a credit score below 400 defaulted and every borrower above 400 did not. In this case, the maximum likelihood estimator does not have a finite solution: the algorithm will attempt to push the coefficient for credit score toward negative infinity, and standard errors will explode.

This is not a rare pathology. It often appears in small datasets or when a binary predictor happens to perfectly align with the outcome in the observed sample. Your software will usually warn you with messages like "fitted probabilities numerically 0 or 1 occurred." When you encounter separation, Firth's penalized likelihood is the most practical remedy for small samples, while Bayesian logistic regression with weakly informative priors is a natural solution in a Bayesian framework.

#### 5. Multicollinearity Inflates Standard Errors

When two or more predictors are highly correlated, the model cannot reliably distinguish their individual effects. In our dataset, income and credit score are likely positively correlated — both tend to be higher for financially stable borrowers. Including both in the model is not wrong, but the standard errors on each coefficient will be inflated, making it harder to detect statistically significant effects even when true associations exist.

You can assess multicollinearity using Variance Inflation Factors (VIFs): a VIF above 5 to 10 for a predictor is a warning sign. The remedy depends on context. Sometimes the correlated predictors can be combined into a single composite measure; other times one of them is genuinely redundant and can be dropped without meaningful loss of predictive power. In all cases, interpret individual coefficients carefully and acknowledge that their estimates are conditional on the other predictors in the model.

---

### Extensions

The following methods extend binary logistic regression for situations where its core assumptions are violated or where additional complexity is warranted. These are not alternatives to learn now — they are signposts for where to look when you outgrow the standard model.

#### Mixed-Effects Logistic Regression

Standard logistic regression assumes all observations are independent. This assumption breaks down when data have a clustered structure — for example, borrowers grouped within branches, patients within hospitals, or students within schools. In these settings, observations within the same cluster tend to be more similar to each other than to observations in other clusters, which violates independence and leads to underestimated standard errors.

Mixed-effects (or multilevel) logistic regression addresses this by including random effects that account for cluster-level variation. The fixed effects still describe the population-average relationships you are interested in, while the random effects absorb the between-cluster differences. In R, the `lme4` package provides `glmer()` for this purpose.

#### Generalized Additive Models (GAMs)

If you suspect that one or more continuous predictors have a nonlinear relationship with the log-odds (see Limitation 1 above), GAMs offer a flexible solution. Instead of specifying a parametric form like a polynomial, GAMs estimate smooth functions of each predictor directly from the data using splines. The result is a model that can capture complex curves while remaining interpretable through plots of the estimated smooth terms. The `mgcv` package in R fits GAMs with a logistic link via `gam(..., family = binomial)`.

#### Penalized Logistic Regression

When the number of predictors is large relative to the sample size, or when predictors are highly correlated, standard maximum likelihood estimation becomes unreliable — estimates have high variance and the model may overfit. Penalized methods add a regularization term to the likelihood that shrinks coefficients toward zero, trading a small amount of bias for a substantial reduction in variance. Ridge regression shrinks all coefficients but retains all predictors; Lasso can shrink some coefficients to exactly zero, performing variable selection at the same time; Elastic Net combines both penalties. In R, the `glmnet` package handles all three through the same interface.

#### Bayesian Logistic Regression

Bayesian logistic regression replaces the single point estimates of maximum likelihood with full posterior distributions over the coefficients. This has several practical advantages: it handles small samples and separation more gracefully, it naturally quantifies uncertainty through credible intervals, and it allows you to incorporate prior knowledge about plausible coefficient values. The `rstanarm` package provides a user-friendly interface that closely mirrors `glm()` syntax, making it a natural next step for analysts already comfortable with standard logistic regression.
