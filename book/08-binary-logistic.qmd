...
# ---- Load dataset (temporary override) ----

use_local_BLR <- TRUE  # set to FALSE once package is updated

if (use_local_BLR) {
  # Read your new CSV (adjust path if you put it in data/)
  BLR <- readr::read_csv("BLR_new.csv", show_col_types = FALSE)
} else {
  # Original behaviour: use dataset from cookbook package
  data("Logistic_Regression")
  BLR <- Logistic_Regression
}

data <- BLR  # keep this for compatibility with later examples
...
# Find region where OLS predictions are < 0
...
if (nrow(neg_region) > 0) {
  ...
  x_min_neg <- min(blr_data$credit_score)
  x_max_neg <- x_min_neg + 50
  x_mid_neg <- (x_min_neg + x_max_neg) / 2
}
...
ggplot(blr_data, aes(x = credit_score, y = defaulted_num)) +
  ...
  geom_hline(yintercept = 1, linetype = "dashed",
             color = "gray50", linewidth = 0.8) +
  # highlight region where predicted probability < 0
  annotate("rect",
           xmin = x_min_neg, xmax = x_max_neg,
           ymin = -Inf, ymax = 0,
...
# Region where predictions are < 0
...
# Highlight impossible region (P < 0)
ax.add_patch(Rectangle((x_min_neg, -0.3),
                       x_max_neg - x_min_neg, 0.3,
                       facecolor='#E74C3C', alpha=0.1, edgecolor='none'))
ax.text(x_mid_neg, -0.15, 'Impossible:\nProbability < 0',
