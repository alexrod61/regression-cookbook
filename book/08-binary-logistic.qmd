...
use_condaenv("regression-cookbook-py", required = TRUE)
...
ggplot(blr_data, aes(x = credit_score, y = defaulted_num)) +
  geom_jitter(height = 0.03, width = 0, alpha = 0.3, color = "black", size = 1.5) +
  ...
  annotate("rect", xmin = 300, xmax = 450, ymin = -Inf, ymax = 0, 
           fill = "#E74C3C", alpha = 0.1) +
  annotate("text", x = 375, y = -0.15, 
           label = "Impossible:\nProbability < 0", 
...
ax.scatter(df['credit_score'], df['defaulted_num'] + jitter, 
           alpha=0.3, color='black', s=20, label='Actual Defaults')

# OLS fitted line
ax.plot(df['credit_score'], df['predicted_ols'], 
        color='#E74C3C', linewidth=2, label='OLS Fit')
...
ax.add_patch(Rectangle((300, -0.3), 150, 0.3, 
                       facecolor='#E74C3C', alpha=0.1, edgecolor='none'))
ax.text(375, -0.15, 'Impossible:\nProbability < 0', 
...
ax.set_title('Why OLS Fails for Binary Outcomes', 
...
The model summary shows that **credit score is highly significant** (p < 0.001), with a coefficient of approximately -0.0065. This tells us that for each 1-point increase in credit score, the log-odds of default decrease by 0.0065, holding income constant. The small p-value gives us strong evidence that this relationship is real, not due to random variation.

In contrast, **income shows weaker evidence** of an independent effect (p ≈ 0.08-0.15, depending on sample). While the coefficient is negative (suggesting higher income reduces default risk), the effect is not statistically significant at the conventional 0.05 level when credit score is already in the model. This suggests that income may not add much predictive power beyond what credit score already captures.

#### Likelihood Ratio Test: Is the Full Model Better?

The likelihood ratio test compares our two-predictor model (credit score + income) against a simpler model with credit score alone. The test yields a chi-squared statistic of approximately 2.68 with 1 degree of freedom (p ≈ 0.10). 

**Interpretation:** The p-value above 0.05 indicates that adding income does **not** significantly improve model fit beyond credit score alone. This aligns with our AIC/BIC results from the Goodness of Fit section, which also favored the simpler model.

#### Confidence Intervals for Odds Ratios

The 95% confidence interval for the credit score odds ratio is approximately [0.987, 0.996]. Because this interval excludes 1.0, we can be confident that credit score has a genuine protective effect against default.

**Plain language translation:** We are 95% confident that each 1-point increase in credit score reduces the odds of default by between 0.4% and 1.3%. Scaled to a more meaningful 50-point increase, this corresponds to roughly a 20-50% reduction in default odds.

The confidence interval for income, however, likely spans across 1.0 (e.g., [0.999, 1.001] per dollar), indicating we cannot confidently say whether income has a positive, negative, or null effect once credit score is accounted for.

#### Practical Implications

These results suggest a **parsimonious model focused on credit score** may be preferable to a more complex model that includes income. From a lender's perspective, this is good news: credit score alone provides sufficient information to assess default risk, simplifying the approval process without sacrificing predictive accuracy.

However, we should interpret this finding cautiously. Income and credit score are likely correlated (higher income often leads to better credit scores), so income's apparent lack of significance may reflect multicollinearity rather than true irrelevance. In a different dataset or with different borrower characteristics, income might play a more important independent role.
...
**Interpretation:** The plot reveals the nonlinear relationship between credit score and default probability. A borrower with a score of 400 faces roughly a 70% chance of default, while someone with a 700 score has only about a 5% chance. Notice how the curve is steepest in the middle (around 500-600), where small improvements in credit score lead to large reductions in default risk. At the extremes (very low or very high scores), additional changes have diminishing marginal effects.
...
sensitivity(results, truth = truth, estimate = predicted)
specificity(results, truth = truth, estimate = predicted)

# Or all at once
metrics <- metric_set(accuracy, sensitivity, specificity, precision, f_meas)
all_metrics <- metrics(results, truth = truth, estimate = predicted)
print(all_metrics)
```
